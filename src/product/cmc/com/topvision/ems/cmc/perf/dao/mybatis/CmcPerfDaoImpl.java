 ...
 * 
 */
@Repository("cmcPerfDao")
public class CmcPerfDaoImpl extends MyBatisDaoSupport<CmcEntity> implements CmcPerfDao {
    ...
    public void recordNoise(SingleNoise singleNoise) {
        /*
         * SingleNoise sn = (SingleNoise) getSqlSession().selectOne(getNameSpace() +
         * "selectLastNoise", singleNoise); if (sn != null) { getSqlSession().update(getNameSpace()
         * + "updateLastNoise", singleNoise); } else { getSqlSession().insert(getNameSpace() +
         * "insertLastNoise", singleNoise); }
         */
    ...
    public List<SingleNoise> getTopLowNoiseLoading(Map<String, Object> paramMap) {
        ...
        return getSqlSession().selectList(getNameSpace() + "selectNoiseRate", paramMap);// modified
                                                                                        // by wubo
                                                                                        // 2017.02.13
    }
    ...
    public void changeCmc8800BStatus(Long cmcId, Boolean status) {
        ...
        if (status) {
            ...
            map.put("ifAdminStatus", CmcConstants.IFOPERSTATUS_UP);
            map.put("ifOperStatus", CmcConstants.IFOPERSTATUS_UP);
        } else {
            map.put("status", CmcConstants.TOPCCMTSSYSSTATUS_OFFLINE);
            map.put("ifAdminStatus", CmcConstants.IFOPERSTATUS_DOWN);
    ...
    public void insertCmcSignalQuality(Long cmcId, String targetName, CmcSignalQualityPerfResult result) {
        ...
        if (PerfTargetConstants.CMC_SNR.equals(targetName)) {
            ...
            for (CmcSignalQuality quality : list) {
                ...
                getSqlSession().update(getNameSpace() + "syncCmcSnrQuality", perfMap);
                // 插入汇总表haojie
    ...
    public void updateCmcStatus(Long cmcId, Integer status) {
        ...
        paramsMap.put("cmcId", cmcId);
        paramsMap.put("cmcStatus", status);
        getSqlSession().update(getNameSpace("updateCmcStatus"), paramsMap);

    }

    @Override
    public void updateCmcSysInfo(CmcServiceQuality cmcServiceQuality) {
        getSqlSession().update(getNameSpace("updateCmcSysInfo"), cmcServiceQuality);
    }
    ...
    public Integer getCmFlapInsCount(Map<String, Object> paramMap) {
        paramMap.put("Authority", CurrentRequest.getUserAuthorityViewName());
        return getSqlSession().selectOne(getNameSpace() + "getCmFlapInsCount", paramMap);
    }
    ...
    @Override
    public List<CmcOpticalTemp> queryCmcOpticalTempInfo(Map<String, Object> paramMap) {
        paramMap.put("Authority", CurrentRequest.getUserAuthorityViewName());
        return this.getSqlSession().selectList(getNameSpace("queryCmcOpticalTempInfo"), paramMap);
    }

    @Override
    public int queryCmcOpticalTempNum(Map<String, Object> paramMap) {
        paramMap.put("Authority", CurrentRequest.getUserAuthorityViewName());
        return this.getSqlSession().selectOne(getNameSpace("queryCmcOpticalTempNum"), paramMap);
    }
