 ...
 * 
 */
@Alias("cmcUpChannelBaseShowInfo")
public class CmcUpChannelBaseShowInfo implements Serializable, AliasesSuperType {
    ...
    private Integer cmtsChannelModulationProfile;// CMTS的上行信道配置模板
    ...
    private String modulationProfileShortName;// 配置的简化形式 仅用QPSK、QAM16、QAM32、QAM64、QAM256表示
    public static final String[] PROFILE_SHORT_NAME = { "", "QPSK", "QAM16", "QAM64", "QAM256", "QPSK", "QAM16",
            "QAM64", "QAM256", "QAM64", "QAM256", "QPSK", "QAM16", "QAM64", "QAM256", "QAM64", "QAM256", "QAM32",
            "QAM32" };

    // @flackyang 用于下行信道配置使用
    private Double upChannelPower;

    public Double getUpChannelPower() {
        if (this.getDocsIf3SignalPower() != null) {
            double upPowerForUnit = this.getDocsIf3SignalPower();
            upChannelPower = UnitConfigConstant.parsePowerValue(upPowerForUnit / 10);
        }
        return upChannelPower;
    }

    public void setUpChannelPower(Double upChannelPower) {
        if (upChannelPower != null) {
            this.docsIf3SignalPower = (int) (UnitConfigConstant.transPowerToDBmV(upChannelPower) * 10);
        }
    ...
    public void setChannelModulationProfile(Long channelModulationProfile) {
        ...
        if (this.channelModulationProfile != null && channelModulationProfile < MODULATIONPROFILETYPES.length) {
            ...
            this.modulationProfileShortName = PROFILE_SHORT_NAME[channelModulationProfile.intValue()];
        } else {
            this.docsIfUpChannelModulationProfileName = "Other";
            this.modulationProfileShortName = "Other";
        }
    }

    public String getModulationProfileShortName() {
        return modulationProfileShortName;
    }

    public void setModulationProfileShortName(String modulationProfileShortName) {
        this.modulationProfileShortName = modulationProfileShortName;
    }
    ...
    public String getDocsIf3SignalPowerForunit() {
        if (docsIf3SignalPower != null) {
            double signalPower = this.docsIf3SignalPower;
            //docsIf3SignalPowerForunit = SignalPower / 10 + " dBmV";
            double powerValue = UnitConfigConstant.parsePowerValue(signalPower / 10);
            this.docsIf3SignalPowerForunit = powerValue + " " + UnitConfigConstant.get("elecLevelUnit");
        }
    ...
    public Integer getCmtsChannelModulationProfile() {
        return cmtsChannelModulationProfile;
    }

    public void setCmtsChannelModulationProfile(Integer cmtsChannelModulationProfile) {
