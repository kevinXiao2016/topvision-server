 ...
 * 
 */
@Service("cmcMacInUsedHandle")
public class CmcMacInUsedHandle extends CmcPerformanceHandle {
    public static String CC_MAC_IN_USED_FLAG = "CC_MAC_IN_USED";
    public static Integer CC_MAC_IN_USED_ALERT_ID = -826;
    public static Integer CC_MAC_IN_USED_EVENT_ID = -827;
    ...
    public PerfThresholdEventParams getEventParams(PerformanceData data) {
        ...
        param.setAlertEventId(CC_MAC_IN_USED_ALERT_ID);
        param.setClearEventId(CC_MAC_IN_USED_EVENT_ID);
        // param.setSource(result.getIfIndex().toString());
        param.setSource("Mac Domain[" + getMac(result.getCmcId()) + "]");
        if (result.getIfInUsed() >= 100) {
            return null;
        }
        Float macUsed = result.getIfInUsed();
        param.setPerfValue(macUsed);
        param.setTargetId(CC_MAC_IN_USED_FLAG);
    ...
    public void initialize() {
        performanceStatisticsCenter.registerPerformanceHandler(CC_MAC_IN_USED_FLAG, this);
    }
    ...
    @Override
    public void destroy() {
        performanceStatisticsCenter.unregisterPerformanceHandler(CC_MAC_IN_USED_FLAG);
    }
