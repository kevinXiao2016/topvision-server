/***********************************************************************
 * $Id: OltPonPortFlowReportCreatorImpl.java,v1.0 2013-10-28 上午10:48:31 $
 * 
 * @author: haojie
 * 
 * (c)Copyright 2011 Topvision All rights reserved.
 ***********************************************************************/
package com.topvision.ems.epon.report.oltponportflow.service.impl;
...
/**
 * @author haojie
 * @created @2013-10-28-上午10:48:31
 * 
 */
@Service("oltPonPortFlowReportCreator")
public class OltPonPortFlowReportCreatorImpl extends BaseService implements OltPonPortFlowReportCreator {
    @Autowired
    private OltPonPortFlowReportDao oltPonPortFlowReportDao;
    @Autowired
    ...
    public void bulidReport(ReportTask reportTask) {
        ...
        if (reportTask.isExcelEnabled()) {
            try {
                Map<String, List<OltPonPortFlowStastic>> statOltPonPortFlow = statPonFlowReport(parMap);
                String filePath = ReportExportUtil.getExportUrl(reportTask, EXCEL);
                OutputStream out = new FileOutputStream(filePath);
                exportPonPortFlowPortToExcel(reportTask, statOltPonPortFlow, statDate, out, conditions);
    ...
    @Override
    public Map<String, List<OltPonPortFlowStastic>> statPonFlowReport(Map<String, Object> map) {
        List<TopoEntityStastic> relates = statReportService.getTopoEntityRelation(entityTypeService.getOltType(), map);
        return oltPonPortFlowReportDao.statPonFlowReport(relates, map);
    }

    @Override
    public void exportPonPortFlowPortToExcel(Map<String, Object> parMap, Date statDate, OutputStream out) {
        Map<String, List<OltPonPortFlowStastic>> statOltPonPortFlow = statPonFlowReport(parMap);
        ...
        exportPonPortFlowPortToExcel(null, statOltPonPortFlow, statDate, out, condition);

    }

    @Override
    public List<OltPonPortFlowDetail> getPonFlowDetail(Map<String, Object> map) {
        return oltPonPortFlowReportDao.selectPonFlowDetail(map);
    }
     ...
     * @param out
     */
    private void exportPonPortFlowPortToExcel(ReportTask task,
            Map<String, List<OltPonPortFlowStastic>> statOltPonPortFlow, Date statDate, OutputStream out,
            String conditions) {
        try {
            Label label;
            // 创建表头等信息
            ...
            int titleCell = 10;
            ReportTaskUtil.formatExcelHeader(sheet, ReportTaskUtil.getString("report.ponFlow", "report"), statDate,
            ...
            for (String folderName : statOltPonPortFlow.keySet()) {
                List<OltPonPortFlowStastic> list = statOltPonPortFlow.get(folderName);
                if (list == null) {
                    continue;
                }
    ...
    @Override
    public void exportPonFlowDetailToExcel(Map<String, Object> map, Date statDate, OutputStream out) {
        List<OltPonPortFlowDetail> list = getPonFlowDetail(map);
        ...
        try {
            ...
            int titleCell = 6;
            ReportTaskUtil.formatExcelHeader(sheet, ReportTaskUtil.getString("report.ponFlowDetail", "report"),
            ...
            WritableCellFormat contentCellFormat = ReportTaskUtil.getContentCellFormat();
            for (OltPonPortFlowDetail oltPonPortFlowDetail : list) {
                label = new Label(0, rowNum, rowNumView.toString(), contentCellFormat);
                sheet.addCell(label);
                label = new Label(1, rowNum, oltPonPortFlowDetail.getPortName(), contentCellFormat);
                sheet.addCell(label);
                label = new Label(2, rowNum, oltPonPortFlowDetail.getName(), contentCellFormat);
                sheet.addCell(label);
                label = new Label(3, rowNum, oltPonPortFlowDetail.getUsage(), contentCellFormat);
                sheet.addCell(label);
                label = new Label(4, rowNum, oltPonPortFlowDetail.getFlowDisplay(), contentCellFormat);
                sheet.addCell(label);
                label = new Label(5, rowNum, oltPonPortFlowDetail.getCollectTimeString(), contentCellFormat);
                sheet.addCell(label);
                rowNum++;
                rowNumView++;
            }
