...
public class FFT {
    ...
    public static Double[] fftshift(Double[] x, int count) {
        Double[] re = new Double[x.length];
        int length = x.length;
        for (int i = 0; i < count ; i++ ) {
            re[length - count + i] = x[i];
        }
    ...
    public static Complex[] fftshift(Complex[] x, int count) {
    ...
    public static void show(Complex[] x, String title,boolean b) {
        System.out.println(title);
        System.out.println("-------------------");
        for (int i = 0; i < x.length; i++) {
            if (b) {
