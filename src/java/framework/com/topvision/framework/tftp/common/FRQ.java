...
/**
 * This class is an abstract class that implements the shared functionality for tftp write and read
 * requests. It only supports the tftp option extensions for timeout and tsize. These are only
 * implemented for server applications and not for client applications
 */
public abstract class FRQ extends TFTPPacket {
    private static Logger log = Logger.getLogger("FRQ.class");

    /**
     * private field contains the tftp options when send in request otherwise null
     */
     ...
     * 
     * @param b
     *            byte array containing tftp request packet
    ...
    /**
     * This constructor is used to construct a WRQ or RRQ object from a byte array received from
     * socket that contains the tftp packet.
     */
    ...
    /**
     * This method is used to return a byte array representing the RRQ or WRQ. The actual OPCODE is
     * set correctly in the derived class. In the server implementation this method will never be
     * called since, RRQ or WRQ will only be received and constructed from byte arrays. Never need
     * this to be converted back to a byte array, except for pure client applications.
     * 
