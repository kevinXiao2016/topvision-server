...
/**
 * class: TFTPUtils package: com.topvision.framework.tftp.common project: tftp4java
 * 
...
public class TFTPUtils {
    ...
    public static ACK dataTransfer(TFTPSocket tftpSock, ACK send, ACK recv) throws IOException {
        ...
        while (receiving) {
            ...
            if (tftpP == null) {
                ...
                tftpLog.debug(getClient(tftpSock) + " expected packet before time out, sending ACK/DATA again");
                tftpSock.write(send);
                ...
                /*try {
                    ...
                    tftpLog.debug(getClient(tftpSock) + "Could not change timeout on socket. " + e.getMessage());
                    // just conitnue
                }*/
                continue;
            }
            ...
            if (tftpP instanceof ACK) {
                // TODO: set timeout back to normal after successfull receival of packet for
                // download
                try {
                    if (stuffedLink) {
                        stuffedLink = false;
    ...
    public static void sendErrPacket(TFTPSocket tftpSock, int errorCode, String errorMsg) {
        ...
        tftpLog.debug("SEND ERROR" + " [" + getClient(tftpSock) + "] EC = [" + errorCode + "] " + errorMsg);
    }
