...
public class ERROR extends TFTPPacket {
    ...
    public ERROR(byte[] tftpP) throws InstantiationException {
        ...
        if (IX_ERRMSG >= tftpP.length) {
            throw new InstantiationException("Argument passed to constructor is not a complete " + getClass().getName()
                    + " packet!");
        }
    ...
    final static String[] errStrings = { "Unknown error", "File not found", "Access violation",
            "Disk full or allocation exceeded", "Illegal TFTP operation", "Unknown transfer ID", "File already exists",
            "No such user" };
