...
import com.topvision.framework.tftp.common.TFTPOptions;
...
/**
 * However this class is called ClientHandler it is meant to be the class that handles the actual
 * client tftp transfers.
 * 
 * @author marco
 */
public class ClientHandler {
    /**
     * Boolean flag that is used internally to indicate we might have stuffed the connection to the
     * client with resent packages. This flag is cleared when we duplicate the timeout with the next
     * data package sent which should help to clean up the channel that did get stuffed
     */
    private boolean stuffedLink = false;

    /**
     * The private field for storage of the timeout setting for the communication with the client
     */
    ...
    /**
     * Connects the clienthandler to the client, packages send do not need to contain a address. If
     * they do they are overwritten by the ones of the connected socket.
     * 
     * @return
     */
    ...
    /**
     * This method returns the IP address and port number of the client this class is talking to.
     */
    ...
    /**
     * This method is an internal helper function that checks if the received ack packet matches the
     * expectations. That means that the opcode and blocknumbers are expected.
     * 
     * @param expecting
    ...
    public boolean receiveFileFromClient(OutputStream os, InetAddress clientAddress, int clientPort, boolean useOptions) {
        ...
        do {
            ...
            try {
                ...
                tftpLog.warn("OutputStream got closed in receiveFileFromClient: " + e.getMessage() + " caused by: "
                        + e.getCause());
                return false;
            }
        ...
        // Because the sendLastAckForDataPacket call can take ages, we close the output stream
        // before sending this ack
        try {
            os.close();
        } catch (IOException e) {
            tftpLog.warn("problem while closing OutputStream in receiveFileFromClient: " + e.getMessage()
    ...
    /**
     * This method will send a file to the client after a read request Because the client buffers
     * are slow to process the package in case of the LSDM it will set a special flag when a package
     * is resent, as soon as the data is acknowledge the next data packet will be sent, it will
     * check and see the special flag, clear the flag and sent for once the new data packet with a
     * doubled time out this gives the LSDM time to also process the possibly delayed data package
     */
    public boolean sendFileToClient(
            /* byte[] fileContent, */
            InputStream is, long totalFileSize, InetAddress clientAddress, int clientPort, String fileName,
            boolean useOptions, EventListener listener) {
        ...
        // 修复无法下载空文件的bug，空文件不应当用TFTPSocket.BLOCK_SIZE去读，这样返回-1，会认为下载失败
        if (totalFileSize == 0L) {
            data = new byte[0];
        }
        ...
        try {
            ...
            if (tftpLog.isTraceEnabled()) {
                tftpLog.trace("tftp useOptions[" + useOptions + "] blockNumber[" + blockNumber + "]");
            }
            if (useOptions) {
                TFTPOptions options = new TFTPOptions();
                options.setTimeout(getTimeout());
            ...
            if (receive == null) {
                if (tftpLog.isTraceEnabled()) {
                    tftpLog.trace("tftp dataTransfer [" + send + "] receive is null");
                }
                return false;
            }
            if (!useOptions && (read < TFTPSocket.BLOCK_SIZE)) {
                if (tftpLog.isTraceEnabled()) {
                    tftpLog.trace("tftp !useOptions[" + useOptions + "] && (read[" + read
                            + "] < TFTPSocket.BLOCK_SIZE[" + TFTPSocket.BLOCK_SIZE + "])");
                }
                return true;
            }
        } catch (ClassCastException e) {
        ...
        do {
            try {
                ...
                if (listener != null) {
                    long m = complete / 1048576L;
                    if (m > mcount) {
                        mcount = m;
                        listener.downloading(clientAddress, clientPort, fileName, totalFileSize, complete);
                    }
                }
    ...
    static Logger tftpLog = LoggerFactory.getLogger(ClientHandler.class);
    ...
    /**
     * Use this method to see if the last ack was sucessfully sent and the data does not need to be
     * resent. If it seems that a new request comes in from the same client we know that the ack was
