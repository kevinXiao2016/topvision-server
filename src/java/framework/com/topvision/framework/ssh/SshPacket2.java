...
public class SshPacket2 extends SshPacket {
    ...
    /**
     * Return the mp-int at the position offset in the data First 4 bytes are the number of bytes in
    ...
    /*
     * 
     * private boolean checkCrc(){ byte[] crc_arrayCheck = new byte[4]; long crcCheck;
     * 
     * crcCheck = SshMisc.crc32(decryptedBlock, decryptedBlock.length-4); crc_arrayCheck[3] = (byte)
     * (crcCheck & 0xff); crc_arrayCheck[2] = (byte) ((crcCheck>>8) & 0xff); crc_arrayCheck[1] =
     * (byte) ((crcCheck>>16) & 0xff); crc_arrayCheck[0] = (byte) ((crcCheck>>24) & 0xff);
     * 
     ...
     * System.err.println(crc_arrayCheck[0]+" == "+crc_array[0]); } if (crc_arrayCheck[3] !=
     * crc_array[3]) return false; if (crc_arrayCheck[2] != crc_array[2]) return false; if
     * (crc_arrayCheck[1] != crc_array[1]) return false; if (crc_arrayCheck[0] != crc_array[0])
