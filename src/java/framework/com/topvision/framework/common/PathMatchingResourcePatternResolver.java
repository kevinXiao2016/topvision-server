 ...
 * may contain the special "<code>classpath*:</code>" prefix and/or internal Ant-style regular
 ...
 * <code>"classpath*:</code>" prefix, and does not contain a PathMatcher pattern, this resolver will
 * simply return a single resource via a <code>getResource()</code> call on the underlying
 * <code>ResourceLoader</code> . Examples are real URLs such as "<code>file:C:/context.xml</code>",
 * pseudo-URLs such as "<code>classpath:/context.xml</code>", and simple unprefixed paths such as "
 * <code>/WEB-INF/context.xml</code>". The latter will resolve in a fashion specific to the
 * underlying <code>ResourceLoader</code> (e.g. <code>ServletContextResource</code> for a
 ...
 * produces a <code>Resource</code> for the path up to the last non-wildcard segment and obtains a
 * <code>URL</code> from it. If this URL is not a "<code>jar:</code>
 * " URL or container-specific variant (e.g. " <code>zip:</code>" in WebLogic, "<code>wsjar</code>
 * " in WebSphere", etc.), then a <code>java.io.File</code> is obtained from it, and used to resolve
 * the wildcard by walking the filesystem. In the case of a jar URL, the resolver either gets a
 * <code>java.net.JarURLConnection</code> from it, or manually parse the jar URL, and then traverse
 ...
 * <code>ResourceLoader</code> is a filesystem one, then wildcarding is guaranteed to work in a
 * completely portable fashion.
 * 
 * <p>
 * If the specified path is a classpath location, then the resolver must obtain the last
 * non-wildcard path segment URL via a <code>Classloader.getResource()</code> call. Since this is
 * just a node of the path (not the file at the end) it is actually undefined (in the ClassLoader
 * Javadocs) exactly what sort of a URL is returned in this case. In practice, it is usually a
 * <code>java.io.File</code> representing the directory, where the classpath resource resolves to a
 * filesystem location, or a jar URL of some sort, where the classpath resource resolves to a jar
 ...
 * <code>java.net.JarURLConnection</code> from it, or manually parse the jar URL, to be able to walk
 * the contents of the jar, and resolve the wildcard. This will work in most environments, but will
 * fail in others, and it is strongly recommended that the wildcard resolution of resources coming
 * from jars be thoroughly tested in your specific environment before you rely on it.
 * 
 * <p>
 ...
 * "<code>classpath*:</code>" prefix. For example, " <code>classpath*:META-INF/beans.xml</code>
 * " will find all "beans.xml" files in the class path, be it in "classes" directories or in JAR
 * files. This is particularly useful for autodetecting config files of the same name at the same
 * location within each jar file. Internally, this happens via a
 * <code>ClassLoader.getResources()</code> call, and is completely portable.
 * 
 * <p>
 * The "classpath*:" prefix can also be combined with a PathMatcher pattern in the rest of the
 * location path, for example "classpath*:META-INF/*-beans.xml". In this case, the resolution
 * strategy is fairly simple: a <code>ClassLoader.getResources()</code> call is used on the last
 ...
 * 
 * <p>
 * <b>WARNING:</b> Note that "<code>classpath*:</code>" when combined with Ant-style patterns will
 ...
 * <code>classpath*:*.xml</code>" will <i>not</i> retrieve files from the root of jar files but
 ...
 * 
 * <p>
 * <b>WARNING:</b> Ant-style patterns with "classpath:" resources are not guaranteed to find
 ...
 * classloader locations, the actual end resource may not be underneath. Therefore, preferably, use
 * "<code>classpath*:<code>" with the same
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
     ...
     * 
     * @param classLoader
     *            the ClassLoader to load classpath resources with, or <code>null</code> for using
     ...
     * location to a <code>java.io.File</code> and passing it into
     * <code>retrieveMatchingFiles</code>, with the remainder of the location as pattern.
     * <p>
     ...
     * <code>doFindPathMatchingJarResources</code> method can handle.
     * <p>
     * The default implementation checks against the URL protocols "jar", "zip" and "wsjar" (the
     * latter are used by BEA WebLogic Server and IBM WebSphere, respectively, but can be treated
     * like jar files).
     * 
     * @param resource
     *            the resource handle to check (usually the root directory to start path matching
     *            from)
     * @see #doFindPathMatchingJarResources
    ...
    protected Resource resolveRootDirResource(Resource original) throws IOException {
        if (equinoxResolveMethod != null) {
            ...
            if (url.getProtocol().startsWith("bundle")) {
                return new UrlResource((URL) ReflectionUtils.invokeMethod(equinoxResolveMethod, null,
                        new Object[] { url }));
            }
        }
        return original;
