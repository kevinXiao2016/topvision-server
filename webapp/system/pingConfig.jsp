...
<script>
	...
	//check retry input
	function checkRetryInput() {
		var reg = /^\d+$/;
		var retriesvalue = Zeta$('pingRetry').value.trim();
		
		if (reg.exec($("#pingRetry").val())
			&& parseInt($("#pingRetry").val()) <= 10
			&& parseInt($("#pingRetry").val()) >= 0) {
			return true;
		} else {
			$("#pingRetry").focus();
			return false;
	...
	//save Ping config
	function okClick() {
		if(checkCountInput() && checkTimeoutInput() && checkRetryInput()){
			window.top.showWaitingDlg("@sys.waiting@", "@sys.saveWaiting@");
			$.ajax({
				url : 'savePingConfig.tv',
				type : 'POST',
...
<body class="openWinBody">
	...
	<form id="pingConfigForm" name="pingConfigForm">
		<div class="openWinHeader">
		    <div class="openWinTip">@sys.setPingConfig@</div>
		        ...
		        <tbody>
		            ...
		            <tr class="darkZebraTr">
		                <td class="rightBlueTxt">
		                   <label for="pingRetry">@sys.pingRetry@:</label>
		                </td>
		                <td>
							<input id="pingRetry" class="normalInput w150"
							name="pingRetry" value="${pingRetry}" maxlength="2"
							toolTip="@sys.pingRetryFocus@" />
		                </td>
		            </tr>
		            <tr>
		                <td class="rightBlueTxt">
