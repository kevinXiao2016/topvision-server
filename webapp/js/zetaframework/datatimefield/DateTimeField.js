...
Ext.DateTimePicker = Ext.extend(Ext.DatePicker, {
    timeFormat: 'g:i:s A',
    timeLabel: Extux.DateTimeField.time,
    timeWidth: 100,
    ...
    onRender: function(container, position){
        ...
        var bt = this.el.child('td.x-date-bottom', true);
        //bt.removeChild(bt.firstChild);
        bt.innerHTML = '<table style="width:100%;"><tr><td style="float:right;" class="x-date-bottom-tdy"></td><td class="x-date-bottom-cfm" ></td></tr></table>'
        if(this.showToday){
            this.todayKeyListener = this.eventEl.addKeyListener(Ext.EventObject.SPACE, this.selectToday,  this);
            var today = (new Date()).dateFormat(this.format);
            this.todayBtn = new Ext.Button({
                renderTo: this.el.child('td.x-date-bottom-tdy', true),
                text: String.format(this.todayText, today),
                tooltip: String.format(this.todayTip, today),
                handler: this.selectToday,
                scope: this
            });
            this.abs = new Ext.Button({
                renderTo: this.el.child('td.x-date-bottom-cfm', true),
                text: this.okText,
                tooltip: String.format(this.todayTip, today),
                handler: function(){
                	var element;
                    if($){
                    	element = $("#"+container.id).find("td.x-date-selected")[0];
                    }else{
                    	element = Ext.DomQuery.selectNode("td.x-date-active.x-date-selected");
                    }
                    if(element != null){
...
Ext.ux.form.DateTimeField = Ext.extend(Ext.form.DateField, {
    ...
    onTriggerClick: function(){
        ...
        //设计思路：在top层增加变量存储一周开始日期。DateTimePicker控件是否也需要统一,nm3kPickDate.js也需要考虑。
        //if(!top.firstDayOfWeek){}
        var theStartDay = this.startDay ? this.startDay : 0;//增加一个星期由第几天开始;
        Ext.apply(this.menu.picker, {
        	startDay: theStartDay,
