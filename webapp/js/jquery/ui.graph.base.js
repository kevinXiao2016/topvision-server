...
$.widget("ui.draggable", $.extend({}, $.ui.mouse, {
    ...
    _generatePosition: function(event) {
        ...
        //添加支持画板缩放的时候拖放坐标计算错误的问题  hujiangyi
        //加入根据缩放比例重新计算坐标的算法
        var pageX = event.pageX/$.ui.mouse._zoomSize;
        var pageY = event.pageY/$.ui.mouse._zoomSize;
        var clicktop = this.offset.click.top/$.ui.mouse._zoomSize;
        var clickleft = this.offset.click.left/$.ui.mouse._zoomSize;
        var xx = 0;
        /*
        ...
        if (this.originalPosition) { //If we are not dragging yet, we won't check for options
            ...
            if (this.containment) {
//                if (pageX - this.offset.click.left < this.containment[0]) pageX = this.containment[0] + this.offset.click.left;
//                if (pageY - this.offset.click.top < this.containment[1]) pageY = this.containment[1] + this.offset.click.top;
//                if (pageX - this.offset.click.left > this.containment[2]) pageX = this.containment[2] + this.offset.click.left;
//                if (pageY - this.offset.click.top > this.containment[3]) pageY = this.containment[3] + this.offset.click.top;
                if (pageX - clickleft < this.containment[0]) pageX = this.containment[0] + clickleft;
                if (pageY - clicktop < this.containment[1]) pageY = this.containment[1] + clicktop;
                if (pageX - clickleft > this.containment[2]) pageX = this.containment[2] + clickleft;
                if (pageY - clicktop > this.containment[3]) pageY = this.containment[3] + clicktop;
            }

            if (o.grid) {
                var top = this.originalPageY + Math.round((pageY - this.originalPageY) / o.grid[1]) * o.grid[1];
                pageY = this.containment ? (!(top - clicktop < this.containment[1] || top - clicktop > this.containment[3]) ? top : (!(top - clicktop < this.containment[1]) ? top - o.grid[1] : top + o.grid[1])) : top;
//                pageY = this.containment ? (!(top - this.offset.click.top < this.containment[1] || top - this.offset.click.top > this.containment[3]) ? top : (!(top - this.offset.click.top < this.containment[1]) ? top - o.grid[1] : top + o.grid[1])) : top;

                var left = this.originalPageX + Math.round((pageX - this.originalPageX) / o.grid[0]) * o.grid[0];
                pageX = this.containment ? (!(left - clickleft < this.containment[0] || left - clickleft > this.containment[2]) ? left : (!(left - clickleft < this.containment[0]) ? left - o.grid[0] : left + o.grid[0])) : left;
        ...
        return {
            top: (
                    pageY                                                                // The absolute mouse position
                            - clicktop                                                    // Click offset (relative to the element)
//                            - this.offset.click.top                                                    // Click offset (relative to the element)
            ...
            left: (
                    pageX                                                                // The absolute mouse position
                            - clickleft                                                // Click offset (relative to the element)
...
$.ui.intersect = function(draggable, droppable, toleranceMode) {
    ...
    //添加支持画板缩放的时候拖放坐标计算错误的问题  hujiangyi
    //加入根据缩放比例重新计算isover的算法
    var left = droppable.offset.left / $.ui.mouse._zoomSize;
    var top = (droppable.offset.top - 27) / $.ui.mouse._zoomSize + 27;
    var x1 = (draggable.positionAbs || draggable.position.absolute).left, x2 = x1 + draggable.helperProportions.width,
		y1 = (draggable.positionAbs || draggable.position.absolute).top, y2 = y1 + draggable.helperProportions.height;
	var l = left, r = l + droppable.proportions.width,
		t = top, b = t + droppable.proportions.height;
