...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    createMenu : function () {
        ...
        if (this.showMenu && hmenu) {
            //////////////////////////////////////////////////////////如果要扩展后台筛选;
        	/*var aFiltersItems = this.filters.items;
        	for(var k=0; k<aFiltersItems.length; k++){
        		if(typeof(this.filters.items[k].clickFn) === 'function'){
	        		
		        	if(hmenu.items.length > 0){
		        		hmenu.items.removeAt(2);
		            	hmenu.items.removeAt(2);
		            }
		        	var paraOptions = this.filters.items[k].options;
		        	var arr = [];
		        	var clickFn = this.filters.items[k].clickFn;
		        	for(var i=0; i<paraOptions.length; i++){
		        		var o = {
		        			text : paraOptions[i].text,
		        			checked : false,
		        			iconCls: 'cirChecked',
		        			handler : function(para){
		        				var outputStr;
		        				if( para.checked == false ){
		        					for(var j=0; j<newMenu.items.items.length; j++){
		        						newMenu.items.items[j].setChecked(false);
		        					}
		        					outputStr = para.text;
		        					hmenu.items.items[2].setChecked(true);
		        				}else{
		        					hmenu.items.items[2].setChecked(false);
		        					outputStr = null;
		        				}
		        				clickFn(outputStr);	
		        			}
		        	...
		        	var newMenu = new Ext.menu.Menu({
		        		id : this.grid.id + 'filters-newMenu',
		        		items : arr
		        	});
		        	...
		        	this.menu = hmenu.add({
		        		iconCls: 'cirChecked',
		        		checked : false,
		        		itemId : 'filterBackData',
		        		text : '全部数据筛选',
		        		menu : newMenu
		        	})
		        	this.menuFilterText = "当前页面筛选";
	        	}
        	}*/
			//////////////////////////////////////////////////////////
