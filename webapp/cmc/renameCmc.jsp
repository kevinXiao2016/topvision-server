...
var renameFlag = '<s:property value="renameFlag"/>';
...
function renameCC() {
	...
	$.ajax({
		  url: '/cmc/renameCc.tv?onuId=' + onuId + '&nmName=' + encodeURI(nmName) + '&cmcId=' + cmcId,
  	      type: 'post',
  	      success: function(response) {
				//response = eval("(" + response + ")");
    	    	if(response.message == "success" && renameFlag > 0){    	    		
    	    		window.parent.showMessageDlg(I18N.COMMON.tip,I18N.CCMTS.renameSuccess);
    	    		window.parent.getFrame("entity-" + entityId).page.changeEntityName(nmName);
				 }else if(response.message == "success" && renameFlag <1){
					 window.parent.showMessageDlg(I18N.COMMON.tip, I18N.CCMTS.renameSuccess);
					 window.parent.getActiveFrame().refreshClick();
				 }else{
					 window.parent.showMessageDlg(I18N.COMMON.tip, I18N.CCMTS.renameError);
					 	}
    	    	closeClick();
