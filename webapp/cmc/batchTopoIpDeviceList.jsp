...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			var cm =  new Ext.grid.ColumnModel([
			    ...
			    {id:"name",header:'<div class="txtCenter">'+'@COMMON.alias@'+'</div>',dataIndex:'name',width: 140,align:'left',sortable:true,renderer:function(value, p, record){
			    	return '<a href="javascript:;" onclick="showEntityDetail(\''+record.data.entityId+'\',\''+record.data.name+'\',\''+record.data.type+'\');">'+value+'</a>';
			    }},
			    ...
			    {header: "@COMMON.manu@", width:120,fixed:true, dataIndex: 'entityId',renderer : manuRender}
			]);
			var tbarArr = [{text: '@IPTOPO.deviceResert@', id: 'refreshAllCm_button', iconCls: 'bmenu_restart', handler: restartCmcList}]
		...
		function manuRender(v,m,r){
			...
			return String.format("<a href='javascript:;' onclick='showEditPage({0},\"{1}\")'>@network/COMMON.editFolder@</a> / <a href='javascript:;' onclick='showCmList({0},\"{1}\")'>@IPTOPO.cmList@</a>", cmcId, name );
		}
		...
		function showCmList(cmcId,name){
			 top.addView('realtimeCmList', '@realtimeCmList@', 'apListIcon',
			            '/realtimecmlist/showRealtimeCmList.tv?c=' + Math.random() + '&cmcId=' + cmcId, null, true);
		}
