   	    ...
   	    //Ext.getCmp('destFileName').regex = null;
	   	    ...
	   	    if (!chooseFileName.length>0){
	        	...
	        	window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.fileSizeIs0, null);  
	           	...
	           	$("#uploadBT").attr("disabled", true).html('<span><i class="miniIcoArrUp"></i><fmt:message bundle="${cmc}" key="CMC.button.uploading"/></span>');	
...
Ext.onReady(function () {
    ...
    flash.onSelect = function(obj){
        ...
        if(obj.size > 100000000){
            ...
            //MODIFY JAY EMS-9219 【EMS-V2.3.2.1-CC8800C-B】：设备快照软件升级中，更改选择的升级文件，保存名未同步更新。 
    ...
    flash.onComplete =function(result){
        ...
        if(result=="updateSuccess"){
            $("#uploadBT").attr("disabled",false).html('<span><i class="miniIcoArrUp"></i><fmt:message bundle="${cmc}" key="CMC.button.updateDevice"/></span>');
            $('#progress').children('.pbar').children('.ui-progressbar-value').css('width', '100%');
        ...
        } else if(result == "download image failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.downloadImgFailed,'error');
        } else if(result == "open image file failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.openImgFailed,'error');
        } else if(result == "length of image file is invalid"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.imageLengthInvalid,'error');
        } else if(result == "check the magic of file failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.checkMagicFileFailed,'error');
        } else if(result == "erase flash failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.eraseFlashFailed,'error');
        } else if(result == "write flash failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.writeFlashFailed,'error');
        } else if(result == "CRC verify failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.CRCVerifyFailed,'error');
        } else if(result == "decompress file failed"){
            window.parent.showMessageDlg(I18N.CMC.tip.tipMsg, I18N.CMC.tip.decompressFileFailed,'error');
        }
    }; */
    ...
    <div class="edgeTB10LR20 pT40">
        ...
        <div class="noWidthCenterOuter clearBoth">
             <ol class="upChannelListOl pB0 pT80 noWidthCenter">
                 <li><a onclick="startUpload()" href="javascript:;" class="normalBtnBig"><span><i class="miniIcoArrUp"></i><fmt:message bundle="${cmc}" key="CMC.button.updateDevice"/></span></a></li>
                 <%-- <li><a  id="uploadBT" onclick="upload()" href="javascript:;" class="normalBtnBig"><span><i class="miniIcoArrUp"></i><fmt:message bundle="${cmc}" key="CMC.button.updateDevice"/></span></a></li> --%>
