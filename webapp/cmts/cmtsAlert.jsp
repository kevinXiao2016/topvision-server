...
<script type="text/javascript"
	src="/include/i18n.tv?modulePath=com.topvision.ems.resources.resources,com.topvision.ems.fault.resources,com.topvision.ems.cmc.resources&prefix=&lang=<%=uc.getUser().getLanguage()%>"></script>
...
function buildPageBox(page) {
	return (''+I18N.entity.alert.PageShowInfo + page + I18N.entity.alert.PageItems);
...
function showAlertDetail(alertId,recordId){
	...
	window.parent.createDialog("modalDlg", I18N.CMC.title.alertProperty, 800, 500, "alert/showAlertDetail.tv?alertId=" + alertId, null, true, true);
}
function onClearAlarmClick() {
	...
	if (sm != null && sm.hasSelection()) {
		...
		window.parent.showTextAreaDlg(I18N.cmcAlert.clear, I18N.entity.alert.alertMessage+':', function (type, text) {
			if (type == "cancel") {
				return;
			}
			if(text.length > 255){
	...
	if (sm != null && sm.hasSelection()) {
		for(var i = 0; i < selections.length; i++){
			if(selections[i].data.status == 1){
				 window.parent.showMessageDlg(I18N.COMMON.tip, "@fault/ALERT.alertConfirmed@");
				 return;
			}
		}
		window.parent.showTextAreaDlg(I18N.entity.alert.ConfirmAlert, I18N.entity.alert.alertMessage, function (type, text) {
		if (type == "cancel") {
			return;
		}
		if(text.length > 255){
...
function confirmAlarm(alertId, status) {
	if(status == 1){
	   window.parent.showMessageDlg(I18N.entity.alert.TipTitle, I18N.entity.alert.alertIsConfirmed);
	   return;
	}
	window.parent.showTextAreaDlg(I18N.entity.alert.ConfirmAlert, I18N.entity.alert.alertMessage, function (type, text) {
        if (type == "cancel") {
...
function renderStatus(value, p, record) {
	if (record.data.status == '1') {
		return String.format('<img hspace=5 src=\"/images/correct.png\" border=0 align=absmiddle nm3kTip="{0}" class="nm3kTip">', I18N.CMC.title.confirmed);
	} else {
		return String.format('<img hspace=5 src=\"/images/wrong.png\" border=0 align=absmiddle nm3kTip="{0}" class="nm3kTip">', I18N.CMC.title.unconfirmed);
...
Ext.onReady(function () {
	...
	var columnModels = [sm,
		{header: '<div class="txtCenter">'+I18N.entity.alert.reason+'</div>', width: 250,align:'left', sortable:false, dataIndex: 'message', renderer: renderNote},
		{header: '<div class="txtCenter">'+I18N.entity.alert.type+'</div>', width: 120, align:'left', sortable:false, dataIndex: 'typeName'},	
		{header: I18N.entity.alert.firstTimeStr, width: 140, sortable:false, dataIndex: 'firstTimeStr'},
		{header: I18N.entity.alert.lastTimeStr, width: 140, sortable:false, dataIndex: 'lastTimeStr'},
		...
		/* {header: I18N.entity.alert.confirmStatus, width: 80, sortable:false, align:'center', dataIndex: 'status', renderer: renderStatus},	
		{header: I18N.entity.alert.confirmUser, width: 80, sortable:false, dataIndex: 'confirmUser'},
		{header: I18N.entity.alert.confirmTime, width: 140, sortable:false, dataIndex: 'confirmTimeStr'}, */
		{header: I18N.CHANNEL.operation, width: 150, align: 'center', sortable:false, dataIndex: 'op', renderer: renderOpeartion,fixed : true}];

	store = new Ext.data.JsonStore({
	    url: '/cmts/alert/getCmtsAlertList.tv?cmcId=' + cmcId,
	    root: 'data', 
	    totalProperty: 'rowCount',
	    fields: ['alertId', 'typeName', 'host', 'levelId', 'levelName', 
	    	'name', 'message', 'firstTimeStr','lastTimeStr', 'status', 'confirmUser', 'confirmTimeStr', 'confirmTime',
	    	'clearUser', 'clearTime', 'clearTimeStr', 'confirmMessage', 'clearMessage']
	});
	...
	grid = new Ext.grid.GridPanel({id: 'extGridContainer',
		border: true, title : I18N.entity.alert.currentAlert,
				...
				<tr>
					...
					<td rowspan="2">
						<ol class="upChannelListOl pB0">
							...
							%>
							<li>
								<a onclick="onConfirmAlarmClick()" id="btn2" href="javascript:;" class="normalBtn"><span><i class="miniIcoCorrect"></i>@entity.alert.ConfirmButton@</span></a>
							</li>
							<%
     						}
         					if (clearAlertPower) {
 							%>
							<li>
								<a onclick="onClearAlarmClick()" id="btn3" href="javascript:;" class="normalBtn"><span><i class="miniIcoClose"></i>@entity.alert.CancelButton@</span></a>
							</li>
							<%
						    }
							%>
