...
<Zeta:Loader>
    ...
    import performance.js.modifyPerfTargetsToTemplate
</Zeta:Loader>
...
.rightPartFloat{ width:600px; float:left; overflow:auto; max-height:116px;}
...
#arrowTip{ width:220px; height:162px; overflow:hidden;position:absolute; top:112px; left:409px; }
.tipPicture{ position:absolute; top:10px; left:80px; z-index:1;}
a.iknow{ position:absolute; top:95px; left:80px; width:100px; height:42px; overflow:hidden; z-index:2; display:block; background:url(../../images/redArrow.png) no-repeat 500px 500px;}
.redArrow{ width:73px; height:28px; overflow:hidden; background:url(../../images/redArrow.png) no-repeat; position:absolute; top:30px; left:0px;
...
var json;
//校验
var maxValue = ${perfTargetJson.maxNum};
var minValue = ${perfTargetJson.minNum};
var regValue = ${perfTargetJson.regexpValue};
var regRule;
</script>
	...
	<div class="edgeTB10LR20 pT5">
		 ...
		 <table class="zebraTableRows" cellpadding="0" cellspacing="0" border="0">
			 <tbody>
				...
				<tr>
					<td></td>
					<td class="vertical-middle">
			    		<input type="checkbox" class="timePeriod weekCbx" value="1"/><span class="mR10 timePeriod">@tip.sun@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="2"/><span class="mR10 timePeriod">@tip.mon@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="3"/><span class="mR10 timePeriod">@tip.thu@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="4"/><span class="mR10 timePeriod">@tip.wed@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="5"/><span class="mR10 timePeriod">@tip.thr@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="6"/><span class="mR10 timePeriod">@tip.fri@</span>
						<input type="checkbox" class="timePeriod weekCbx" value="7"/><span class="mR10 timePeriod">@tip.sat@</span>
					</td>
				</tr>
			 </tbody>
		</table>
		<div class="noWidthCenterOuter clearBoth">
			 <ol class="upChannelListOl pB0  pT20 noWidthCenter">
		     	 <li><a href="javascript:;" class="normalBtnBig" onclick="resetDft()"><span><i class="miniIcoBack"></i>@tip.resetDefault@</span></a></li>
		         <li><a href="javascript:;" class="normalBtnBig" onclick="confirm()"><span><i class="miniIcoEdit"></i>@tip.modify@</span></a></li>
	...
	<script type="text/javascript">
		...
		function showSubInput(json){
			...
			for(var i=0; i<len; i++){
					 ...
					 str +=     '<li>'+ rangeStr +'</li>';
					 str +=     '<li><input type="text" class="normalInput w50" value="'+ config[i].value +'" maxlength="10"/></li>';
					 ...
					 str +=     '<li><input type="text" class="normalInput w50" value="'+ clearRules[i].value +'" maxlength="10"/></li>';
					 str +=     '<li class="unitSpan">'+ json.unit +'</li>';
					 str +=     '<li class="lastLi">';
		...
		function renderRange(num){
			var aRange = [{
				...
				rangeStr = '<select class="normalSel">';
		...
		function addOneLine(){
			...
			if($oneLine.length < page.lineNum){
				...
				str += '<ul class="leftFloatUl clearBoth pB5 oneLine">';
  				var rangeStr = renderRange(0);
				str +=     '<li>'+ rangeStr +'</li>';
				str +=     '<li><input type="text" class="normalInput w50" value="" maxlength="10"/></li>';	
				jsonUnit = json.unit;
