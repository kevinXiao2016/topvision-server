...
function showEntityDetail(entityId, entityName){
	window.parent.addView('entity-' + entityId, entityName , 'entityTabIcon','/epon/oltPerfGraph/showOltPerfViewJsp.tv?module=7&entityId=' + entityId, null, true);
}

function deviceNameRender(value, p, record){
	var formatStr = '<a href="#" onclick="showEntityDetail({0},\'{1}\')">{1}</a>';
	return String.format(formatStr, record.data.entityId, record.data.entityName);
}

function ipRender(value, p, record){
	var formatStr = '<a href="#" onclick="showEntityDetail({0},\'{1}\')">{2}</a>';
	return String.format(formatStr, record.data.entityId, record.data.entityName, record.data.entityIp);
}
...
$(document).ready(function() {
	...
	var cm =  new Ext.grid.ColumnModel([
	    ...
	    {id:"entityIp",header:'<div class="txtCenter">@Performance.manageIp@</div>',dataIndex:'entityIp',align:'left',width:100, renderer: ipRender},
	...
	store = new Ext.data.JsonStore({
	    url: '/epon/perfThreshold/loadOltThresholdTemplate.tv',
	    root: 'data', 
...
function onSeachClick() {
	...
	if($("#entityIp").val()!=""){
		...
		queryObject.entityIp = $("#entityIp").val();
	}
...
function showTemplateDetail(templateId){
	...
	$("#frameOuter").show();
	var m = Math.random();
	$("#frameInner").attr("src",'/performance/perfThreshold/showModifyTemplate.tv?templateId='+templateId+"&m="+m+'&originalFrame=frameoltPerfTemp');
}