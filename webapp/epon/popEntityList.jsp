...
Ext.onReady(function() {
	...
	var sm = new Ext.grid.CheckboxSelectionModel({
	    singleSelect: true,
        header:'<div class="x-grid3-hd-checker" id="c_grid"></div>'    
	});
	<%
	} else {
	%>
    ...
    var cm = new Ext.grid.ColumnModel([
    	sm,
    	{header: '<fmt:message bundle="${report}" key="report.deviceName" />', dataIndex: 'name', sortable: true, width: 200},
	...
	grid = new Ext.grid.GridPanel({width: 624, height: 290, border: true,
        ...
        bbar: new Ext.PagingToolbar({pageSize: pageSize, store: store, displayInfo: true, items: ['-','<fmt:message bundle="${report}" key="COMMON.pageShow" />25<fmt:message bundle="${report}" key="COMMON.size" />']}),
        renderTo: 'grid-div'
    });
	store.load({params: {start: 0, limit: pageSize}});
	
	store.on("load",function(){
        $("#c_grid").parent().removeClass("x-grid3-hd-checker-on");
    });
...
<table width=100% cellspacing=0 cellpadding=0>
	<tr><td width=80px height=26px><fmt:message bundle="${report}" key="report.topo" />:</td><Td>
		<select id="folderId" style="width:172px" name="folderId">
			<option value="0"><fmt:message bundle="${report}" key="report.allEntity" /></option>
...
<div id="buttonPanel" align=right style="padding-top:10px;display:none">
	...
	onMouseOut="this.className='BUTTON75'" onMouseDown="this.className='BUTTON_PRESSED75'"
	onclick="okClick()"><fmt:message bundle="${report}" key="COMMON.confirm" /></button>&nbsp;
