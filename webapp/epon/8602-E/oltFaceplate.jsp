        ...
        if(slotType == "fan"){
            ...
            if (portIndex == 0) {//board
               if (UPLINK_BOARD.contains(slotType) || CONTROL_BOARD.contains(slotType)) {//GE
                ...
                if ((portIndex > 8 && slotType == "meub") || (portIndex > 16 && slotType == "meua")) {//SNI口
                	// modify by fanzidong,找到对应端口
                	var sniPort = null;
                	for(var i=0; i<slot.portList.length; i++) {
                		if(slot.portList[i].portRealIndex == portIndex) {
                			sniPort = slot.portList[i];
                			break;
                		}
                	}	
...
function beginConstructTree(){
	try {
		var root = new Ext.tree.TreeNode({text : "PN8602-E",id : "OLT",expanded : true,icon : '/images/network/pn8602-E_16.png'});
		var slotName = null;
		for(var i = 1; i < olt.slotList.length + 1; i++) {
			var slotTemp = olt.slotList[i - 1];
			
			// 8602E/EF不需要判断预配置类型，只有一个板卡，也不存在板卡插拔
			...
			for(var j = 1; j < $portList.length + 1; j++) {//否则就绘制端口节点
				...
				//-----------对应的是全局流通的divId----------//
				var portId = bType[slotTemp.topSysBdPreConfigType] + "_" + slotTemp.slotRealIndex + "_" + portTemp.portRealIndex;
				var portImage;
				//-----得到需要展示的图标和portName-----//
...
//覆盖原因：所有的端口都是epua，设备说明里面无法显示SNI口。实际上17口到26口都是SNI口;
					...
					<div id="NotationsContainer">
						<div class="edge5 DeviceNotation">
							<table class="dataTable zebra noWrap" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#DFDFDF" rules="all">
								<tbody>
								    <tr>
								        <td class="rightBlueTxt">MEUA/MEUB:</td>
								        <td>@EPON.geua@</td>
								        <td class="rightBlueTxt">MEUA/MEUB:</td>
								        <td>@EPON.mpua@</td>
										<td class="rightBlueTxt">MEUA/MEUB:</td>
