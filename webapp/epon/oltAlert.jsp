...
<%
	...
	boolean supportGZ = uc.hasSupportProject("gz");
%> 
...
function alertTypeChanged(obj) {
	if ( $(obj).val() == 1) {
		location.href = '/epon/alert/showOltAlert.tv?module=5&type=1&entityId=' + entityId + '&entityType=olt'
	}  else {
		location.href = '/epon/alert/showOltAlertHistory.tv?module=5&type=3&entityId=' + entityId + '&entityType=olt'
	}
}
...
function onShowSubAlertClick() {
	window.top.addView('alertViewer', '@resources/EVENT.alarmViewer@', "icoF2", 'alert/showCurrentAlertList.tv?level=0&hostDevice=' + entityIp,false,true);
}
	...
	if (sm != null && sm.hasSelection()) {
		...
		window.parent.showTextAreaDlg(I18N.oltAlert.confirmAlert, "@resources/EVENT.confirmMessage@<br />@COMMON.remarks@", function (type, text) {
			...
			Ext.Ajax.request({url:"/fault/confirmAlert.tv", method:"post",
				...
				}, params:{alertIds: alertIds,entityId : entityId, message: text}})
		})
	} else {
		window.parent.showMessageDlg(I18N.COMMON.message, I18N.oltAlert.selectAlertData)
	}
...
function doClearOperation(alertId){
	window.parent.showTextAreaDlg(I18N.oltAlert.cleanAlert, "@resources/EVENT.clearMessage@<br />@COMMON.remarks@", function (type, text) {
        ...
        }, params:{alertIds: [alertId],entityId : entityId, message:text}})
    })
}
					...
					<tr>
						...
						<td rowspan="3" colspan="2" style="vertical-align: middle;text-align: left;">
							<ol class="upChannelListOl pB0">
								...
								<% if(supportGZ) { %>
								<li>
									<a id="btn3" href="javascript:;" class="normalBtn " onclick="onShowSubAlertClick()"><span><i class="miniIcoSearch"></i>@oltAlert.checkSubAlert@</span></a>
								</li>
								<% } %>
							</ol>
						</td>
