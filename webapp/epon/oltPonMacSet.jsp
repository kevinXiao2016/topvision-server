...
function checkValid() {
	...
	var reg = /^([0-9])+$/;
	if (!( (reg.exec(maxLearningNum) && maxLearningNum > 0 && maxLearningNum <= 32767) || maxLearningNum == -1) ) {
		$("#ponMaxNum").focus();
		return false;
	}
	return true;
...
function cancelClick() {
	window.parent.closeWindow('ponMacLearningNumSet');
}
function authLoad(){
		...
		<table class="mCenter zebraTableRows" >
			<tr>
				...
				<td class="withoutBorderBottom">
					<input id="ponMaxNum" maxlength="5"  class="normalInput w130" tooltip="@SERVICE.macMaxLearnTip@" />
				</td>
			</tr>
		</table>
	</div>
