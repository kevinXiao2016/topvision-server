...
.openLayerProfileMain{ width:560px; height:280px; overflow:hidden; position:absolute; top:80px; left:120px; z-index:2;  background:#F7F7F7;}
...
<script type="text/javascript">
	...
	var srvBindCap = '${srvBindCap}';
	
	//关闭当前弹出框
 	function closeBtClick(){
 		window.parent.closeWindow('profileVlanConfig');
 	}
	...
	//添加模板
	function addProfile(){
		var vlanProfileId = $("#vlanProfileId").text();
	...
	$(document).ready(function(){
		//当ONU业务模板已经绑定能力集时,不能再添加IGMP模板
		if(srvBindCap > 0){
			$("#addProFile").hide();
		}
		window.dataStore = new Ext.data.JsonStore({
			url : '/epon/businessTemplate/loadVlanProfile.tv',
			fields : ["vlanProfileId", "srvPortId", "bindVlanProfile","profileVlanPvid","profileVlanPvidPri", "srvBindCap"],
			baseParams : {
				entityId : entityId,
				profileId : profileId
			}
		...
		var cmArr = [ 
		 	{header : "@TEMPL.srvProfileId@",width : 80,align : 'center',dataIndex : 'vlanProfileId'}, 
		 	{header : "@TEMPL.portId@",width : 80,align : 'center',dataIndex : 'srvPortId'}, 
		 	{header : "@TEMPL.uniVlanProfile@",width : 80,align : 'center',dataIndex : 'bindVlanProfile'},
		...
		cmArr.push({header : "@COMMON.manu@",width : 60, align : 'center',dataIndex :'op', renderer : opeartionRender})
