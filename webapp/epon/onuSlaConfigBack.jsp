		...
		success : function(response) {
			if(response.responseText != 'success'){
				top.nm3kRightClickTips({
    				...
    				html: I18N.SLA.refreshSlaEr
    			});
				return;
			}
			top.nm3kRightClickTips({
				title: I18N.COMMON.tip,
				html: I18N.SLA.refreshSlaOk
			});
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				if(response.responseText != 'success'){
					top.nm3kRightClickTips({
						title: I18N.COMMON.tip,
						html: I18N.SLA.mdfOnuSlaEr
					});
					return;
				}
				top.nm3kRightClickTips({
					title: I18N.COMMON.tip,
					html: I18N.SLA.mdfOnuSlaOk
				});
				cancelClick();
			},
			failure : function() {
...
function cancelClick() {
	window.parent.closeWindow('onuSlaConfigBack');
}
...
function keyup(s){
	...
	if(dscir!=null && dscir!="" && dspir!=null && dspir!="" && usfir!=null && usfir!="" && uscir!=null && uscir!="" && 
		...
		}else if(parseInt(usfir) > 750000){
			$("#message7").attr("color","red");	
		}else if(parseInt(uscir) > 950000){
			$("#message8").attr("color","red");
		}else{
		...
		<div class="edgeTB10LR20 pT20">
			<table class="zebraTableRows">
				<tr>
					...
					<td width="230"><input id="dsCir" type=text value="0" onkeyup="keyup(1)" class="normalInput"
				...
				<tr>
					...
					<td colspan="3">
						<input id="usPir" type=text value="10000" onkeyup=keyup(5)  class="normalInput"
						maxlength=7 tooltip='@SLA.range1000000@' /> Kbps
					</td>
				</tr>
				<tr class="darkZebraTr">
					...
					<td colspan=3 class="txtLeft">
						...
						<p class="pT10">@SLA.tip7@ @SLA.down@:<font color=green id=leaveDsCir>1000000</font>Kbps&nbsp;&nbsp;&nbsp;&nbsp;@SLA.up@:<font color=green id=leaveUsCir>1000000</font>Kbps</p>
					</td>
				</tr>
			</table>
		</div>
