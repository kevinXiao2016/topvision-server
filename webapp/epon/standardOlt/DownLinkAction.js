function detailHandler() {
    ...
    var cmcName = entity.cmcList[0].entityName;
...
function workModeHandler() {
    ...
    window.top.createDialog('uniAutoNegotiation', 'UNI@ONU.workMode@', 600, 410, '/onu/showUniAutoNegoConfig.tv?&onuId=' +
            page.entityId + '&uniId=' + getActualId() + '&entityId=' + entityId + "&realIndex=" + realIndex, null, true, true);
}
...
function bindCameraFn(){
	 //计算参数信息
    var slot = entity.onuPonPort.slotNo;
    var port = entity.onuPonPort.ponNo;
    //---------解析ONU-INDEX-----------//
    var onuindex = entity.onuIndex.toString(16);
    if (onuindex.length==9)
        onuindex = '0'.concat(onuindex);
    onuindex  = parseInt(onuindex.substring(4,6),16);
    var displayOnu = slot + '/' + port + ':' + onuindex;
