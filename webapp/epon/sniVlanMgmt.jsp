...
var vlanTagTpid = '<s:property value="portVlanAttribute.vlanTagTpidString"/>';
...
function doOnload(){
	...
	if(vlanTagTpid!=""&&vlanTagTpid!=null){
		$("#vlanTagTpidSelect").val(vlanTagTpid);
	}else{
		$("#vlanTagTpidSelect").val(0);
		$("#vlanTagTpidSelect").val($("#vlanTagTpidSelect").find("option:selected").text());
	}
...
function saveClick() {
	 ...
	 var vlanTagTpid = $("#vlanTagTpidSelect").val();
	 /* 
	 if(!checkTpid()){
	 ...
	 $.ajax({
	        ...
	        type: 'POST',
	        data: "entityId=" + entityId + "&sniIndex=" + sniIndex +"&sniId=" + sniId + "&vlanTagPriority=" + vlanTagPriority + "&vlanTagTpid=" + vlanTagTpid +"&vlanPVid=" + pvid + "&num=" + Math.random(),
	        dataType:"text",
...
function changeValue(){
	//vlanTagTpidSelect.value=vlanTagTpidSelect.value
	var vlanTagTpidSelect = $("#vlanTagTpidSelect").val();
	if(vlanTagTpidSelect == 0){
		$("#vlanTagTpidSelect").val($("#vlanTagTpidSelect").find("option:selected").text());
	}else{
		$("#vlanTagTpidSelect").val($("#vlanTagTpidSelect").val());
	}
}
function authLoad(){
...
<body class=openWinBody onload="doOnload();authLoad();">
	...
	<div class="edge10">
		...
		<tr class="darkZebraTr">	
			...
			<td>
				<select id="vlanTagTpidSelect" onchange="changeValue();" class="w200 normalSel">
		...
		<tr>	
			...
			<td><input type="text" name="vlanMode" value="hybrid" class="normalInput w200" disabled /></td>
		</tr>
	</table>
	</form>
	</div>
