...
<Zeta:HTML xmlns="http://www.w3.org/1999/xhtml" vmlSupport="true">
<head>
...
<style type="text/css">
#portletTools .x-panel-body,#configFileMgmt .x-panel-mc,#tools ,#portletPerformace .x-panel-body{ background:#fff;}
...
function onAddToGoogleMaps() {
	...
	/* window.parent.createDialog("googleDlg", I18N.OLT.addGoogleMap, 600, 450, "google/add2GoogleMap.tv?entityId =" + entityId, null, true, true,
          function(){
               if (window.top.ZetaCallback.type == 'cancel') {return;}
            Ext.Ajax.request({url: '/google/saveEntity2GoogleMap.tv',
                ...
                success: function() {},
                failure: function(){window.parent.showMessageDlg(I18N.COMMON.err, I18N.OLT.addGoogleMapFail, 'error');}
            })
...
function loadPonUsedInfo(){
	 $.ajax({
	        ...
	        dataType:'json',
	        success: function(json) {
//	            $("#ponTotalNum").text(json.ponTotalNum);
...
//上行端口用户数统计
var ponCmNumColumns = [{header: '@EPON.PortName@', width: 100, sortable: false, align: 'center', dataIndex: 'ponPortName', renderer: renderPonPortName},
...
//上行端口用户数统计
var cmtsCmNumColumns = [{header: '@EPON.cmtsName@', width: 100, sortable: false, align: 'center', dataIndex: 'cmtsName'/* , renderer: renderCmtsName */},
...
function refreshCmList(){
    window.parent.showConfirmDlg("@COMMON.tip@", "@network/NETWORK.confirmRefreshCmList@", function(button,text){
        if(button == "yes"){
            window.parent.showWaitingDlg("@COMMON.waiting@", "@network/NETWORK.refreshCmList@",'waitingMsg','ext-mb-waiting');
            $.ajax({
                url:'/cmlist/refreshCmList.tv?entityId='+entityId,
                type:'POST',
                dateType:'json',
                success:function(response){
                    if(response == "success"){
                        ...
                        top.afterSaveOrDelete({
                            title: '@COMMON.tip@',
                            html: '<b class="orangeTxt">@network/NETWORK.refreshCmListSuccess@</b>'
                        });
                        //window.parent.onRefreshClick();
                        if(top.getFrame("entity-"+entityId)!=null){
                            top.contentPanel.setActiveTab("entity-"+entityId);
                            window.location.reload();
                        }
                    }else{
                        window.parent.showMessageDlg("@COMMON.tip@", "@network/NETWORK.refreshCmListFail@");
                    }
                },
                error:function(){
                    window.parent.showMessageDlg("@COMMON.tip@", "@network/NETWORK.refreshCmListFail@");
...
Ext.onReady(function(){
	...
	if(VC.support("mpuaBDRestore")){
		$("#masterSwitch").show();
	}
	//基本信息;
	var portletDetail = {
	    id:'portletDetail',
	    title: I18N.ENTITYSNAP.basicInfo.basicInfo,
	...
	var oltLocation = {
		id:'oltLocation',
        title: I18N.ENTITYSNAP.basicInfo.deviceIpInfo,
        bodyStyle: 'padding:10px',
        autoScroll: true,
        contentEl:'location'
    }
  ...
  //上行端口用户数统计
	...
	var portletTools = {
		id:'portletTools',
        title: I18N.ENTITYSNAP.tools.tools,
        bodyStyle:'padding:10px',
        autoScroll:true,
        contentEl:'tools'
	}
	...
	var portletSysUpTime = {
		id:'portletSysUpTime',
        title: I18N.ENTITYSNAP.deviceUpTime.deviceUpTime,
        bodyStyle: 'padding:10px',
        autoScroll: true,
	...
	//主控板最近性能;
	var portletPerformace = {
	        id:'portletPerformace',
	        title: I18N.ENTITYSNAP.performance.mpu,
	        bodyStyle:'padding:0px',
	        autoScroll:true,
	        contentEl:'cpuandmemperformance'
	    }
 	var columns = 2;
    var portletItems = [];
    //左侧板块，从数据库读取;
	...
	if(leftPartStr == '' && rightPartStr == ''){
        if (isSupportCmc) {
            leftPartStr = "portletDetail,oltLocation,subDeviceInfo,portletCmInfo,ponUsedInfo,ponCmNum,cmtsCmNum";
    ...
    setInterval(refreshPerformance,180000);
    ...
    //操作权限控制，主要是控制管理工具里的按钮
  	if(!operPower){
  	    $("#sysTime").attr("disabled",true);
  	 	...
  	 	$("#switchHandler").attr("disabled",true);
  	 	$("#topoGraph").attr("disabled",true);
  	 	$("#setDolPolling").attr("disabled",true);
  	 	/* $("#onuGlobalConfig").attr("disabled",true); */
  	}
    ...
    if(str.length > 0){
    	...
    	for(var i=0,len=arr.length; i<len; i++){
    		newStr += '<span style="margin:0px 10px 0px 0px;">' + arr[i] + '</span>';
    	}
			...
			<div style="position:relative">
				...
				<table class="dataTable zebra" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#DFDFDF" rules="all">
					<tr>   
						...
						<td class="txtLeftEdge wordBreak"><div class="pR120"><s:property value="oltAttribute.oltName"/></div></td>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.alias@:</td>					
						<td class="txtLeftEdge wordBreak"><div class="pR120"><s:property value="entity.name"/></div></td>
					    
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.deviceStyle@:</td>
						<s:if test="oltAttribute.oltDeviceStyle == 1">
						<td class="txtLeftEdge wordBreak"><div class="pR120">@ENTITYSNAP.basicInfo.fixed@</div></td>
						</s:if>
						<s:if test="oltAttribute.oltDeviceStyle == 2">
						<td class="txtLeftEdge wordBreak"><div class="pR120">@ENTITYSNAP.basicInfo.chassisBased@</div></td>
						</s:if>
						<s:else>
							<td class="txtLeftEdge wordBreak"><div class="pR120">@ENTITYSNAP.basicInfo.unknownType@</div></td>
						</s:else>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.type@:</td>
						<td class="txtLeftEdge wordBreak"><div class="pR120"><s:property value="oltAttribute.oltType"/></div></td>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.vender@:</td>
						<td class="txtLeftEdge wordBreak"><div class="pR120"><s:property value="oltAttribute.vendorName"/></div></td>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.boards@:</td>
						<td class="txtLeftEdge"><s:property value="slotNumInUse"/> @ENTITYSNAP.basicInfo.zaiwei@,
						@ENTITYSNAP.basicInfo.gong@ <s:property value="oltAttribute.oltDeviceNumOfTotalServiceSlot"/></td>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.powers@:</td>
						<td class="txtLeftEdge"><s:property value="powerNumInUse"/> @ENTITYSNAP.basicInfo.zaiwei@,
						@ENTITYSNAP.basicInfo.gong@ <s:property value="oltAttribute.oltDeviceNumOfTotalPowerSlot"/></td>
					</tr>
					<tr>
						<td class="rightBlueTxt">@ENTITYSNAP.basicInfo.fans@:</td>
						<td class="txtLeftEdge"><s:property value="fanNumInUse"/> @ENTITYSNAP.basicInfo.zaiwei@,
						@ENTITYSNAP.basicInfo.gong@ <s:property value="oltAttribute.oltDeviceNumOfTotalFanSlot"/></td>
					</tr>
					<tr>
