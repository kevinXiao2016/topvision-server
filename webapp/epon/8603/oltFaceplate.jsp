...
<Zeta:Loader>
    ...
    IMPORT epon.8603.oltCreationLib
...
function beginConstructTree(){
	try {
		var root = new Ext.tree.TreeNode({text : "PN8603",id : "OLT",expanded : true,icon : '/images/network/8603_16.gif'})
		var slotName = null
		for(var i = 1; i < olt.slotList.length + 1; i++) {
			...
			//@fix @BUG EMS-6447【EMS-V2.0.0.0】实际板卡获取类型为unknown时，左侧树状导航栏无对应槽位显示 BY @Bravin
			...
			if(preType == type && preType != SLOT_BOARD) {
            	...
            	// 有预配置类型并且和实际类型不一样，这时分两种：一种是没有插入板卡，一种是插入了板卡但和预配置类型不一致  
                ...
                if(slotTemp.topSysBdActualType == 0) {
                    slotName = I18N.EPON.slot + "_" + slotTemp.slotRealIndex
                    //---------------pre == 0 != act ---------------//
                } else {
			...
			var slotId = preType + "_" + slotTemp.slotRealIndex + "_0"
			//---------construct new TreeNode------------//
			var slotnode = new Ext.tree.TreeNode({
				text : slotName,id : slotId,expanded : true,draggable : false,icon : '/images/network/port/port_48.gif',
				listeners : {
			...
			for(var j = 1; j < slotTemp.portList.length + 1; j++) {//否则就绘制端口节点
				...
				//-----------对应的是全局流通的divId----------//
				var portId = preType + "_" + slotTemp.slotRealIndex + "_" + j
				var portImage
				//-----得到需要展示的图标和portName-----//
...
function beginConstructPage() {
	...
	//------加载olt数据----------//
	Ext.Ajax.request({
        url:"../epon/oltObjectCreate.tv?entityId=" + entityId, method:"post",
        success:function (response) {
        	$(".loadingText").html(I18N.EPON.dataBack + "..")
            try{
				...
				olt.initOltPortOptical();
	           	beginConstructTree()
	        	$(".loadingText").html(I18N.EPON.drawing + "...")
...
/******************************************************************
							数据操作区
 *****************************************************************/
function clearOltFaceplate() {
	// 清除结构树
    $("#viewLeftPartBody").empty();
    // 清除面板区域
            ...
            if (portIndex == 0) {
                if (CONTROL_BOARD.contains(slotType)) {//主控板属性
                    ...
                    entityObject = {
                        ...
                        '@EPON.installedType@' : bType[_slot.topSysBdActualType].toUpperCase()+tempString,
...
function fanConfigChange(fanId, level){
	var tmpSource = grid.getSource();
	tmpSource['@EPON.fanSpeed@'] = I18N.EPON.collecting
		...
		Ext.Ajax.request({
			...
			success: function (text) {
				if(text.responseText=='false' || text==null){
					...
					tmpSource['@EPON.fanSpeed@'] = topSysFanSpeedToRpm(text.responseText);
	...
	<div class="wrapWH100percent overfloatHidden">
		...
		<div id="viewMiddle" class="viewMiddle">
			...
			<div id="viewMiddleBottom" class="viewMiddleBottom threeFeBg">
				...
				<div id="viewMiddleBottomBody" class="displayNone overflowAuto">
								...
								<dd class="twoPartDlLeft">
									<table class="dataTableRows" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#DFDFDF" rules="rows">
									     ...
									     <tbody>
									         ...
									         <tr><td class="pL10">@EPON.fiveRow@</td></tr>
										</tbody>
									</table>																		
								</dd>
								<dd class="twoPartDlRight">
									<table class="dataTableRows" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#DFDFDF" rules="rows">
									     ...
									     <tbody>
									         ...
									         <tr>
									         	<td width="16" style="padding-left:10px"><div class="critical"></div></td>
		                           		 		<td> <font>critical @EPON.alert@</font></td>                       		 	                       		 	
		                           		 		<td width="16" style="padding-left:10px"><div class="major"></div></td>
		                           		 		<td> <font>major @EPON.alert@</font></td>
									         </tr>
									         <tr>
									         	<td  style="padding-left:10px"><div class="minor"></div></td>
		                           		 		<td> <font>minor @EPON.alert@</font></td>      
		                           		 		<td style="padding-left:10px"><div class="warning"></div></td>
		                           		 		<td> <font>warning @EPON.alert@</font></td>
									         </tr>
									         <tr>
