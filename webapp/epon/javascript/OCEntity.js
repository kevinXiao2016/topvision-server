...
OCEntity.prototype.initializeONU = function(obj){
	...
	this.onuEorG = obj.onuEorG
    this.entityType = page.entityType
    this.onuMac = obj.onuUniqueIdentification || 0 //TODO WHERE DEFINED IT IN MIB
    ...
    this.topOnuHardwareVersion = obj.topOnuHardwareVersion || ''
    ...
    this.onuRunTime = obj.onuRunTime || '--'
    this.laserSwitch = obj.laserSwitch || 2
    //PON PORT
    ...
    this.hasCmc = obj.cmcList.length
    //CMC结点初始化,适用于8800A的初始化
	for(var i=0 ;i<obj.cmcList.length;i++){
		var o = this.cmcList[i] = new Object()
		o.entityId = obj.cmcList[i].entityId || 0
	    o.cmcEntityId = obj.cmcList[i].cmcEntityId || 0
	    o.cmcIndex = obj.cmcList[i].cmcIndex || 0
        o.cmcId = obj.cmcList[i].cmcId || 0
        //不在这里进行状态的处理,在获取oltonuAttribute中状态时从entitysnap表中获取
        //this.onuOperationStatus = obj.cmcList[i].topCcmtsSysStatus || 2;
        o.cmcMac = obj.cmcList[i].topCcmtsSysMacAddr || ""
        //o.typeId = obj.cmcList[i].cmcType || ""
        o.onuId = obj.cmcList[i].onuId || 0
	    o.onuIndex = obj.cmcList[i].onuIndex || 0
	    this.entityType = o.entityType = obj.cmcList[i].cmcType || 0
	    o.entityName = obj.cmcList[i].cmcName || "@ONU.cmc8800b@"
	    o.cmcDeviceStyle = obj.cmcList[i].cmcDeviceStyle || 0//TODO WHICH
	    o.sysDesc = obj.cmcList[i].sysDesc || ''
	    o.sysObjectID = obj.cmcList[i].sysObjectID || 0
	    o.sysUpTime = obj.cmcList[i].topCcmtsSysUpTime || 0
	    o.sysName = obj.cmcList[i].sysName || ''
	    o.sysLocation = obj.cmcList[i].sysLocation || ''
	    o.sysServices = obj.cmcList[i].sysServices || ''//TODO WHICH
	    o = null
	}
}
...
OCEntity.prototype.createComponent = function(idx, entityId_, Lpx, Tpx, Wid, heig, img, pDiv,title,notModify){
	...
	if('onuDevice' == entityId_) {
		$entity.attr('id' , entityId_).css('z-index', 10).addClass("onuClass");
...
OCEntity.prototype.drawEntity = function(){
	...
	switch(parseInt(this.entityType)){//CC,ONU
	    ...
	    /* CATV-WIFI ONU **/
		case 37:
			if($.inArray(this.topOnuHardwareVersion, ["8625-004","8625-102","8625-103","8625-121","8625-127","8625-302","8625-405"]) != -1){
				IMG.ONUDEVICE = '/epon/image/onu/8625-102.png'; //4FE室外..目前也使用室外型的外壳
				this.drawOnu_8625_102(4);
			}else if($.inArray(this.topOnuHardwareVersion, ["8625-202","8625-203","8625-223","8625-225","8625-227","8625-402"]) != -1){
				IMG.ONUDEVICE = '/epon/image/onu/8625-202.png'; //4FE室外..目前也使用室外型的外壳
				this.drawOnu_8625_202(4);
			}
			break;
		case 38:
			if($.inArray(this.topOnuHardwareVersion, ["8626-102","8626-107","8626-121","8626-124","8626-125","8626-127","8626-502","8626-602"]) != -1){
				IMG.ONUDEVICE = '/epon/image/onu/8626-102.png'; //4FE室外..目前也使用室外型的外壳
				this.drawOnu_8626_102(4);
			}else if($.inArray(this.topOnuHardwareVersion, ["8626-302","8626-307","8626-321","8626-323","8626-327","8626-402","8626-702","8626-704","8626-802"]) != -1){
				IMG.ONUDEVICE = '/epon/image/onu/8626-302.png'; //4FE室外..目前也使用室外型的外壳
				this.drawOnu_8626_302(4);
			}
			break;
		case 65://8FE
...
OCEntity.prototype.drawCMC_A = function(){
    ...
    }else if(EntityType.isCC8800FS($typeId)){   //TODO F型，待支持，先用CE图片代替
        cmcImage = '/epon/image/cmc/icmc-f.png';
    }else{
		cmcImage = IMG.iCMCDEVICE;
	}
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", 150, -80, 280, 190, cmcImage, "entityContainer");
	this.ui = this.createComponent(0, "onuDevice", 150, -80, 280, 190, '', "entityContainer");
	...
	if(this.onuOperationStatus == 1){
		$('<div class="greenGIF" style="width:97px; height:105px; top:73px; left:102px; position:absolute; z-index:1"></div>').appendTo($("#PanelContainer"));
	}else{
		$('<div class="redGIF" style="width:97px; height:105px; top:73px; left:102px; position:absolute; z-index:1"></div>').appendTo($("#PanelContainer"));
	}
	...
	//---------解析ONU-INDEX-----------//
	var vmlPort = "@ONU.portNum@:" + this.getLocation();
	
	var vmlLight1,vmlLight2,vmlLight3,vmlLight4;
	var onuTrans = this.onuPonPort.onuTransPower == 0 ?"--":this.onuPonPort.onuTransPower;
	var onuRev = this.onuPonPort.onuRevPower == 0?"--":this.onuPonPort.onuRevPower;
	var oltPonTrans = this.oltPonOptical.transPower == 0 ?"--":this.oltPonOptical.transPower.toFixed(2);
	
	if(this.onuOperationStatus == 1){
		vmlLight1  = "@ONU.sendOptRate@:" + onuTrans + ' dBm';
		vmlLight2  = "@ONU.recOptRate@:" + onuRev + ' dBm';
	}else{
		vmlLight1 = "@ONU.sendOptRate@:-- dBm";
		vmlLight2 = "@ONU.recOptRate@:-- dBm";
	}
	vmlLight3  = "@ONU.ponport@@ONU.sendOptRate@:" + oltPonTrans + ' dBm';
	//----绘制参数信息-----//
	$vmlText = $("<div>");
...
OCEntity.prototype.drawOnu_8641 = function(){
	...
	//draw onu(cmc),cmc
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", -20, -30, 454, 75, IMG.ONUDEVICE, "entityContainer");
	this.ui = this.createComponent(0, "onuDevice", -20, -30, 454, 75, '', "entityContainer")
	//onu pon
	...
	var onuTrans = this.onuPonPort.onuTransPower == 0?"--":this.onuPonPort.onuTransPower;
	var onuRev = this.onuPonPort.onuRevPower == 0?"--":this.onuPonPort.onuRevPower;
	var vmlLight1 = "@ONU.sendOptRate@:" + onuTrans + ' dBm';
	var vmlLight2 = "@ONU.recOptRate@:" + onuRev + ' dBm';
	//----绘制参数信息-----//
	$vmlText = $("<div>");
	$vmlText.css("zindex",500000)
	//端口号
...
OCEntity.prototype.drawOnu_8644 = function(){
	...
	//draw onu(cmc),cmc也要当onu看,太扯淡了
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", -20, -30, 454, 54, IMG.ONUDEVICE, "entityContainer");
	this.ui = this.createComponent(0, "onuDevice", -20, -30, 454, 54, '', "entityContainer")
	//onu pon
	this.execute(function(){
...
OCEntity.prototype.drawOnu_8647 = function(){
	...
	//draw onu(cmc),cmc也要当onu看,太扯淡了
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", -20, -30, 454, 56, IMG.ONUDEVICE, "entityContainer");
	this.ui = this.createComponent(0, "onuDevice", -20, -30, 454, 56, '', "entityContainer")
	//onu pon
...
OCEntity.prototype.drawOnu_GENERAL = function(portSize){
	...
	///DEVICE
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", 0, 0, 451, 74, IMG.ONUDEVICE, "entityContainer");
	this.execute(function(){this.ui = this.createComponent(0, "onuDevice", 0, 0, 451, 74, "", "entityContainer");});
	///PON PORT
	this.execute(function(){
	...
	var vmlLight1 = "@ONU.sendOptRate@:" + onuTrans + ' dBm';
	var vmlLight2 = "@ONU.recOptRate@:" + onuRev + ' dBm';
	var vmlLight3  = "@ONU.ponport@@ONU.sendOptRate@:" + oltPonTrans + ' dBm';
	//----绘制参数信息-----//
	...
	//修改参数描述的位置信息
	$("#vmlInterface").css({
		position:'absolute',left:235,top:44
	});
	$("#vmlInterfaceSend").css({
		position:'absolute', left:225,  top:80
	});
	$("#vmlSender").css({
 ...
 * @portSize NumberOfPorts
 ***********************************************/
OCEntity.prototype.drawOnu_8626_102 = function(portSize){
	portSize = portSize || this.uniSize ;
	///DEVICE
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", 0, -200, 451, 274, IMG.ONUDEVICE, "entityContainer");
	this.execute(function(){this.ui = this.createComponent(0, "onuDevice", 0, -200, 451, 274, '', "entityContainer");});
	///PON PORT
	this.execute(function(){
		this.onuPonPort.ui = this.createComponent(1, "onuPon_0", 115, 212, 23, 23, IMG.PONIMG, "onuDevice");
	},{},null,function(){
		this.onuPonPort.ui = this.createComponent2(0, "onuPon_0" , 115, 212, 23, 23, IMG.PONIMG, "onuDevice");
	})
	var portIndex = -1;
	///UNI PORTS
	while(++portIndex < portSize){
		var portRealIndex = 4-portIndex;
		this.execute(function(){
			if(!this.onuUniPortList[portIndex].collected){throw error();}
			this.onuUniPortList[portIndex].ui = this.createComponent(0, "onuUni_" + portRealIndex, 165 + portIndex * 37, 212, 23, 23, IMG.UNIIMG, "onuDevice");
			var uni = this.onuUniPortList[portRealIndex-1];
			this.changeUniState(uni.uniAdminStatus,uni.uniOperationStatus,"onuUni_" + portRealIndex);
		},{},null,function(){
			//可能导致下标越界
			/*this.onuUniPortList[portIndex].*/
			var ui = this.createComponent2(0, "onuUni_" + portRealIndex, 165 + portIndex * 37, 212, 23, 23, IMG.UNIIMG, "onuDevice");
		});
	};
	//DRAW  SPLITER & UPDOWN LINE
	$spliter = $("<div class='spliterClass' onuOperationStatus=2></div>");
	...
	//draw OLT icon
	this.execute(function(){this.olt = this.createComponent(0, "oltPanel", 313, 22, 50, 50, oltImg, "PanelContainer");});
	//计算参数信息
	var vmlSlot = this.onuPonPort.slotNo;
	var vmlPonPort = this.onuPonPort.ponNo;
	//---------解析ONU-INDEX-----------//
	...
	//修改参数描述的位置信息
	$("#vmlInterface").css({
		position:'absolute',left:245,top:34
	});
	$("#vmlInterfaceSend").css({
		position:'absolute', left:245,  top:70
	});
	$("#vmlSender").css({
		position:'absolute',left:165,top:160
	});
	$("#vmlReveicer").css({
		position:'absolute',left:165,top:180
	})
	$("#vmlInterface").html(vmlPort);
	$("#vmlSender").html(vmlLight1);
	$("#vmlReveicer").html(vmlLight2);
	...
	//光接受功率
	this.text.light2 = $("#vmlLight2");
	$("#entityType").css({top:300, left:32});
}

OCEntity.prototype.drawOnu_8626_302 = function(portSize){
	portSize = portSize || this.uniSize ;
	///DEVICE
	// modify by fanzidong, 增加背景图
	// modify by fanzidong, 增加背景图
	...
	///PON PORT
	this.execute(function(){
		this.onuPonPort.ui = this.createComponent(1, "onuPon_0", 115, 210, 23, 23, IMG.PONIMG, "onuDevice");
	},{},null,function(){
		this.onuPonPort.ui = this.createComponent2(0, "onuPon_0" , 115, 210, 23, 23, IMG.PONIMG, "onuDevice");
	})
	var portIndex = -1;
	///UNI PORTS
	while(++portIndex < portSize){
		...
		this.execute(function(){
			if(!this.onuUniPortList[portIndex].collected){throw error();}
			this.onuUniPortList[portIndex].ui = this.createComponent(0, "onuUni_" + portRealIndex, 165 + portIndex * 37, 210, 23, 23, IMG.UNIIMG, "onuDevice");
			var uni = this.onuUniPortList[portRealIndex-1];
			this.changeUniState(uni.uniAdminStatus,uni.uniOperationStatus,"onuUni_" + portRealIndex);
		},{},null,function(){
			//可能导致下标越界
			/*this.onuUniPortList[portIndex].*/
			var ui = this.createComponent2(0, "onuUni_" + portRealIndex, 165 + portIndex * 37, 210, 23, 23, IMG.UNIIMG, "onuDevice");
		});
	};
	//DRAW  SPLITER & UPDOWN LINE
	$spliter = $("<div class='spliterClass' onuOperationStatus=2></div>");
 ...
 * @portSize NumberOfPorts
 ***********************************************/
OCEntity.prototype.drawOnu_8625_102 = function(portSize){
	portSize = portSize || this.uniSize ;
	///DEVICE
	// modify by fanzidong 
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", 0, -109, 451, 274, IMG.ONUDEVICE, "entityContainer");
	this.execute(function(){this.ui = this.createComponent(0, "onuDevice", 0, -109, 451, 274, '', "entityContainer");});
	///PON PORT
	this.execute(function(){
		this.onuPonPort.ui = this.createComponent(1, "onuPon_0", 70, 205, 23, 23, IMG.PONIMG, "onuDevice");
	},{},null,function(){
		this.onuPonPort.ui = this.createComponent2(0, "onuPon_0" , 70, 205, 23, 23, IMG.PONIMG, "onuDevice");
	})
	var portIndex = -1;
	///UNI PORTS
	while(++portIndex < portSize){
		...
		this.execute(function(){
			if(!this.onuUniPortList[portIndex].collected){throw error();}
			this.onuUniPortList[portIndex].ui = this.createComponent(0, "onuUni_" + portRealIndex, 138 + portIndex * 44, 205, 23, 23, IMG.UNIIMG, "onuDevice");
			var uni = this.onuUniPortList[portRealIndex-1];
			this.changeUniState(uni.uniAdminStatus,uni.uniOperationStatus,"onuUni_" + portRealIndex);
		},{},null,function(){
			//可能导致下标越界
			/*this.onuUniPortList[portIndex].*/
			var ui = this.createComponent2(0, "onuUni_" + portRealIndex, 137 + portIndex * 44, 206, 23, 23, IMG.UNIIMG, "onuDevice");
		});
	};
	//DRAW  SPLITER & UPDOWN LINE
	$spliter = $("<div class='spliterClass' onuOperationStatus=2></div>");
	...
	$updown = $("<img>")
	$updown.attr({
		src:'/epon/image/onu/spliter2.gif',
		id:'onuUpDown', width: 225, height:265
	}).css({
		position: 'absolute',
		left: -48,  top: -2
	}).addClass('updownClass').appendTo($spliter);
	...
	//修改参数描述的位置信息
	$("#vmlInterface").css({
		position:'absolute',left:225,top:24
	});
	$("#vmlInterfaceSend").css({
		position:'absolute', left:225,  top:70
	});
	$("#vmlSender").css({
		position:'absolute',left:125,top:247
	});
	$("#vmlReveicer").css({
		position:'absolute',left:125,top:266
	})
	$("#vmlInterface").html(vmlPort);
	$("#vmlSender").html(vmlLight1);
	$("#vmlReveicer").html(vmlLight2);
	...
	//光接受功率
	this.text.light2 = $("#vmlLight2");
	$("#entityType").css({top:390, left:52});
}

OCEntity.prototype.drawOnu_8625_202 = function(portSize){
	portSize = portSize || this.uniSize ;
	///DEVICE
	// modify by fanzidong, 增加背景图
	// modify by fanzidong, 增加背景图
	this.createComponent(0, "onuDevice_bg", 0, -100, 451, 274, IMG.ONUDEVICE, "entityContainer");
	this.execute(function(){this.ui = this.createComponent(0, "onuDevice", 0, -100, 451, 274, '', "entityContainer");});
	///PON PORT
	this.execute(function(){
		this.onuPonPort.ui = this.createComponent(1, "onuPon_0", 70, 205, 23, 23, IMG.PONIMG, "onuDevice");
	},{},null,function(){
	...
	$updown.attr({
		src:'/epon/image/onu/spliter2.gif',
		id:'onuUpDown', width: 225, height:285
	}).css({
		position: 'absolute',
		left: -48,  top: -14
	}).addClass('updownClass').appendTo($spliter);
	...
	$("#vmlSender").css({
		position:'absolute',left:125,top:255
	});
	$("#vmlReveicer").css({
		position:'absolute',left:125,top:275
	})
	$("#vmlInterface").html(vmlPort);
	$("#vmlSender").html(vmlLight1);
	$("#vmlReveicer").html(vmlLight2);
...
OCEntity.prototype.startListen = function(){
	...
	this.ui && this.ui.bind('mouseover', function(e) {
...
OCEntity.prototype.showStatus = function(){
	...
	if(this.onuOperationStatus == 1){
		$("#newtree A[onuid='"+this.entityId+"']").addClass("onlineLink").removeClass("offlineLink");
	}else{
		$("#newtree A[onuid='"+this.entityId+"']").removeClass("onlineLink").addClass("offlineLink");
	}
	...
	if(entity.onuOperationStatus == 1){
		...
		if(!EntityType.isCcmtsType(this.entityType)){
			if(this.entityType === 37){
				$("#onuUpDown").attr('src','/epon/image/onu/spliterDown2.gif')	
			}else{
...
OCEntity.prototype.getLocation = function(){
    var vmlSlot = this.onuPonPort.slotNo;
    var vmlPonPort = this.onuPonPort.ponNo;
    //---------解析ONU-INDEX-----------//
    var onuindex = this.onuIndex.toString(16);
    if(vmlSlot == 0){
    	if (onuindex.length==7){
    		onuindex = '000'.concat(onuindex);
    	}else if(onuindex.length==6){
    		onuindex = '0000'.concat(onuindex);
    	}else{
    		onuindex = '00'.concat(onuindex);
    	}
    }else{
        if (onuindex.length==9){
    		onuindex = '0'.concat(onuindex);
    	}else if(onuindex.length==8){
    		onuindex = '00'.concat(onuindex);
    	}
    }
    onuindex  = parseInt(onuindex.substring(4,6),16);
    return vmlSlot + '/' + vmlPonPort + ':' + onuindex;
}
...
OCEntity.prototype._showComponentProperty = function(nodeId) {
    ...
    if(nodeId != 'oltPanel') {
        if(nodeId.substring(3, 6).toLowerCase() == 'pon') {
            ...
            for(a=1; a<5; a++){
                tmp[a] = tmp[a] && tmp[a] != 0 ? (parseInt(tmp[a]) / 100) + tmpC[a] : "--";
            }
            tmp[5] = tmp[5] && tmp[5] != 0 ? (parseInt(tmp[5]) / 100000) + tmpC[5] : "--";
            entityStore = { 
            ...
            if(this.onuEorG == "G"){
            	delete entityStore["@ONU.flowcontrolEnable@"];
            	delete entityStore['@ONU.autonegEnable@'];
            }
            ...
            if(oltSoftVersion.indexOf("1.6.9.9-p4") != -1 && !EntityType.isCcmtsType(this.entityType) ){
            	...
            	if(!EntityType.isCcmtsType(this.entityType)){
            		if(this.onuOperationStatus == 1){
            			...
            			upTime = this.onuRunTime;
            		}
            	}else{
            		if(this.onuOperationStatus == 4 || this.onuOperationStatus == 1){
            ...
            if(this.onuEorG == "G"){
            	delete entityStore["@ONU.chipvender@"];
            	delete entityStore["@ONU.chipType@"];
            	delete entityStore["@ONU.chipVersion@"];
            	delete entityStore["@ONU.macAddr@"];
            	delete entityStore["LLID"];
            	entityStore['SN'] = this.onuMac.replaceAll(":","").toUpperCase();
            }
        }
