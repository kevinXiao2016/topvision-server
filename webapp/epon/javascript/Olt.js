...
var Olt = function(){
	...
	o.getItem = function(slot,port){
	    slot--;
	    if(port){
	        port--;
	        return o.slotList[slot].portList[port];
	    }
	    return o.slotList[slot];
	};
	...
	o._init = function(json){
		...
		this.sysLocation = json.sysLocation || " - ";
		...
		this.systemRogueCheck = json.systemRogueCheck || 2;
		...
		// modify by fanzidong，适配不同OLT类型的槽位数，改善代码可读性
		switch(this.oltType) {
		case 'PN8601':
			this.slotCount = 18;
			break;
		...
		//this.slotCount = this.oltType == 'PN8601'?18:this.oltType == 'PN8602'?3:this.oltType == 'PN8603'?8:(this.oltType=='PN8602-E')?1:18;
			...
			switch(json.slotList[slotCursor].slotRealIndex-1 == cursor){
				case true:
					...
					var _slot = json.slotList[slotCursor];
						...
						slot.slotDisplayTemp = _slot.slotDisplayTemp || 0;
						...
						for (var cursorB=0,portCursor=0; cursorB < _slot.portList.length; cursorB++) {
							...
							switch((_slot.portList[portCursor].portRealIndex-1 == cursorB) || (_slot.portList[portCursor].portRealIndex-9 == cursorB)){
								case true:
										...
										port.ponRogueSwitch = _port.ponRogueSwitch || 2;
...
//				fan.valid = false;
			...
			default:
				...
				//胡乔说的 -1 表示没有告警，0是最高级别的告警
				fan.fanCardAlarmStatus = _fan.fanCardAlarmStatus || 0;
		...
		for (var cursor=0; cursor < json.powerList.length; cursor++) {
			...
			var _power = json.powerList[cursor];
				...
				power.powerDisplayTemp = _power.powerDisplayTemp || 0;
				//--------powerDC(1), powerAC(2)-------------//
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				if(tmpFlag == opticalFlag && tmpIndex > -1){
					...
					tmpSource[I18N.Optical.identifier] = data[0] && parseInt(data[0]) > 0 && parseInt(data[0]) < 4 ? fiberType[data[0]] : null;
