...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//移出已经添加了WAN的选项
		for (var i = 0; i < wanIdList.length; i++) {
			$("#connectId option[value=" + wanIdList[i] + "]").remove();
		}
	...
	function cancelClick() {
		window.parent.closeWindow('addWanConnect');
	}
	...
	function save() {
		...
		window.parent.showWaitingDlg("@COMMON.wait@", '@ONU.WAN.new@' + 'WAN' + '@ONU.WAN.connection@', 'waitingMsg', 'ext-mb-waiting');
		$.ajax({
			url : '/onu/addWanConnect.tv',
			type : 'POST',
			data : {
				entityId : entityId,
				onuId : onuId,
			...
			success : function(response) {
				...
				if(response == 'success'){
					...
					window.parent.showMessageDlg(I18N.COMMON.tip, '@ONU.WAN.new@' + '@ONU.WAN.failed@');
				}
			},
			error : function() {
				window.parent.showMessageDlg(I18N.COMMON.tip, '@ONU.WAN.new@' + '@ONU.WAN.failed@');
...
<title>@ONU.WAN.new@WAN@ONU.WAN.connection@</title>
</head>
<body class="openWinBody">
	<div class="edge10 pT5 pB5">
		<ul class="leftFloatUl pB5">
			...
			<li>
				<select id="connectId" class="normalSel w165">
		...
		<div class="zebraTableCaption clearBoth pT20 pB10">
			...
			<table class="zebraTableRows" cellpadding="0" cellspacing="0" border="0" id="zebraTable">
				<tr class="withoutBorderBottom">
					...
					<td>
						<select id="ipModeSelect" class="normalSel w165" onchange="ipModeSelectChanged()">
				...
				<tr class="withoutBorderBottom">
					<!-- <td class="rightBlueTxt channelExtModeName">@ONU.WAN.connectName@:</td>
					<td class="channelExtModeName"><input
						class="normalInput w165" id="connectName"
						tooltip='@ONU.WAN.ConnectNameTip@' /></td> -->
