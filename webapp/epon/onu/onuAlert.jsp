...
function initData() {
	// 初始化查询条件
	$.ajax({
		url: '/epon/alert/loadAlertQueryInitData.tv',cache:false,dataType:'json',
	...
	//初始化告警类型
	$.ajax({
		url: '/onu/getOnuAlertType.tv',cache:false,dataType:'json',
		success: function(json) {
		...
		callback: function(records) {
			if (records && records.length == 0) {
				top.nm3kAlertTips({
					...
					html:"<b class='orangeTxt'>@oltAlert.noAlertData@</b>",
...
function showPrintWnd(obj, doc) {
	...
	doc.write('<html>')
	doc.write('<head>')
	doc.write('<title>@oltAlert.deviceAlert@[${entity.ip}"/>]</title>')
...
function onClearAlarmClick() {
	...
	if (sm != null && sm.hasSelection()) {
		...
		window.parent.showTextAreaDlg(I18N.oltAlert.cleanAlert, "@resources/EVENT.clearMessage@<br />@COMMON.remarks@", function (type, text) {
			if (type == "cancel") {
				return
			}
			if( !V.isRemarks(text) ){
...
function onConfirmAlarmClick() {
	...
	if (sm != null && sm.hasSelection()) {
		...
		window.parent.showTextAreaDlg(I18N.oltAlert.confirmAlert, "@resources/EVENT.confirmMessage@<br />@COMMON.remarks@", function (type, text) {
			if (type == "cancel") {
				return
			}
			if( !V.isRemarks(text) ){
...
function renderStatus(value, p, record) {
	if (record.data.status == '1') {
		return String.format('<img hspace=5 src=\"/images/fault/confirm.png\" border=0 align=absmiddle nm3kTip="{0}" class="mm3kTip">', I18N.oltAlert.checked);
	} else {
		return String.format('<img hspace=5 src=\"/images/fault/unconfirm.png\" border=0 align=absmiddle nm3kTip="{0}" class="mm3kTip">', I18N.oltAlert.noCheck);	
	}
}
function getLastAlertId() {
	var model =  grid.getSelectionModel();
...
function doClearOperation(alertId){
	window.parent.showTextAreaDlg(I18N.oltAlert.cleanAlert, "@resources/EVENT.clearMessage@<br />@COMMON.remarks@", function (type, text) {
        ...
        if( !V.isRemarks(text) ){
			window.parent.showMessageDlg(I18N.COMMON.tip, "@resources/alert.confirmAlertMessage2@");
			return;
		}
        Ext.Ajax.request({url:"/fault/clearAlert.tv", method:"post", success:function (response) {
            onRefreshClick()
...
$(document).ready(function () { 
	...
	var columnModels = [sm,
     		{header: '<div class="txtCenter">@oltAlert.alertReason@</div>',width:250, sortable: false, dataIndex: 'message', align: 'left', renderer: renderNote},
     		{header: '<div class="txtCenter">@oltAlert.type@</div>', sortable: false, align: 'left', dataIndex: 'typeName'},	
     		{header: "@oltAlert.createTime@",  sortable: false, dataIndex: 'firstTimeStr'},
     		{header: "@oltAlert.lastTime@", sortable: false, dataIndex: 'lastTimeStr'},
     		...
     		{header: "@COMMON.manu@",width:150,  sortable:false, dataIndex: 'op', renderer: manuRenderer , fixed : true}
     	];
    store = new Ext.data.JsonStore({
	    url: '/onu/loadOnuAlert.tv',
	    baseParams: {
	    	type: type,
	    	entityId:${onu.parentId},
	    	onuId: onuId
	    ...
	    fields: ['alertId', 'typeName', 'host', 'levelId', 'levelName', 'name', 'message', 'firstTimeStr','lastTimeStr','status', 'confirmUser', 'confirmTime', 'confirmTimeStr', 'clearUser', 'clearTime', 'clearTimeStr', 'confirmMessage', 'clearMessage']
	});
	...
	store.load({params:{start:0, limit: pageSize, alertId : alertId},
		callback: function(records) {
