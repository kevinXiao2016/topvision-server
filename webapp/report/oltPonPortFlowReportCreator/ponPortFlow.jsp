...
<%@ page import="com.topvision.ems.epon.report.domain.OltPonPortFlowStastic" %>
...
function query(){
	...
	location.href = "/epon/report/queryPonFlowStastic.tv?stTime=" + stTime + "&etTime="+etTime;	
}
...
function exportExcelClick() {
	var stTime = stField.value;
	var etTime = etField.value;
	window.location.href="/epon/report/exportPonReportToExcel.tv?stTime=" + stTime + "&etTime=" + etTime; 
}

function showDetail(rangeStart,rangeEnd,entityId){
	var stTime = stField.value;
	var etTime = etField.value;
	var url = "/epon/report/queryPonFlowDetail.tv?stTime=" + stTime + "&etTime=" + etTime+"&entityId="+entityId+"&rangeStart="+rangeStart+"&rangeEnd="+rangeEnd;
	window.top.addView("ponFlowDetail"+Math.random(),"@report.ponFlowDetail@",null,url);
}
...
<body class="whiteToBlack">
	...
	<div class="report-content-div" id="pageview">
		...
		<table class="reportTable" id="reportTable">
			...
			<tbody>
				<%
				    Map<String, List<OltPonPortFlowStastic>> statSniFlowReport = (Map<String, List<OltPonPortFlowStastic>>) request
				                .getAttribute("statPonFlowReport");
				        ...
				        for (String folderName : statSniFlowReport.keySet()) {
				            List<OltPonPortFlowStastic> list = statSniFlowReport.get(folderName);
				            if (list == null) {
				%>
				<tr>
					<td><%=folderName%></td>
				...
				<%
				                ...
				                for (int i = 0; i < list.size(); i++) {
				                    OltPonPortFlowStastic stastic = list.get(i);
