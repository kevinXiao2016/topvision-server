...
<%@ page import="com.topvision.ems.epon.report.domain.HistoryAlarmReport" %>
...
function query() {
	...
	queryForm.action = '/epon/report/showHistoryAlarmReport.tv?query=true&stTime=' + stTime + '&etTime=' +etTime;
...
function showAlarmDetail(folderId, entityId, alarmTypeId){
	var stTime = stField.value;
	var etTime = etField.value;
	var url ="/epon/report/showHistoryAlertReportDetail.tv?stTime=" + stTime + "&etTime=" + etTime; 
	if(folderId!=''){
	...
	url += "&eponSelected=" + $("#eponSelected").is(":checked"); 
	url += "&ccmtsSelected=" + $("#ccmtsSelected").is(":checked"); 
	window.parent.addView("historyAlertReportDetail"+ Math.random(),"@report.historyAlarmDetailStatic@",'reportIcon', url);
}
...
function exportExcelClick() {
	...
	window.location.href="/epon/report/exportHistoryAlertReportToExcel.tv?stTime=" + stTime + "&etTime=" + etTime + "&eponSelected=" + 
...
<body class=whiteToBlack>
	...
	<div class="report-content-div" id="pageview">
		<h1>@report.historyAlarmStatic@</h1>
		...
		<table class="reportTable" id="reportTable">
			<tbody>
				...
				Map<Long, FolderEntities> folderEntities 
					= (Map<Long, FolderEntities>)request.getAttribute("historyAlarmReport");
				...
				for(Long folderId : folderEntities.keySet()){
	 		 	  ...
	 		 	  %>
	 		 	  			<tr>
	 		 	  				...
	 		 	  				<td><%=historyAlarmReport.getEntityName()%></td>
	 		 	  				<td class="nowrap"><%=historyAlarmReport.getDisplayName()%></td>
	 		 	  				<td>
								<%if(historyAlarmReport.getAllAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick="showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '')">
										<%=historyAlarmReport.getAllAlarmNum()%>
									</a>
								<%} %>
								</td>
								<td>
								<%if(historyAlarmReport.getEmergencyAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '6')">
										<%=historyAlarmReport.getEmergencyAlarmNum()%>
									</a>
								<%} %>
								</td>
								<td>
								<%if(historyAlarmReport.getSeriousAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '5')">
										<%=historyAlarmReport.getSeriousAlarmNum()%>
									</a>
								<%} %>
								</td>
								<s:if test="mainAlarmDisable">
								<td>
								<%if(historyAlarmReport.getMainAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '4')">
										<%=historyAlarmReport.getMainAlarmNum()%>
									</a>
								<%} %>
								</td>
								</s:if>
								<s:if test="minorAlarmDisable">
								<td>
								<%if(historyAlarmReport.getMinorAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '3')">
										<%=historyAlarmReport.getMinorAlarmNum()%>
									</a>
								<%} %>
								</td>
								</s:if>
								<s:if test="generalAlarmDisable">
								<td>
								<%if(historyAlarmReport.getGeneralAlarmNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '2')">
										<%=historyAlarmReport.getGeneralAlarmNum()%>
									</a>
								<%} %>
								</td>
								</s:if>
								<s:if test="messageDisable">
								<td>
								<%if(historyAlarmReport.getMessageNum()==0){ %>
									0
								<%}else{ %>
									<a href="#" onclick = "showAlarmDetail('<%=folderId%>', '<%=historyAlarmReport.getEntityId()%>', '1')">
										<%=historyAlarmReport.getMessageNum()%>
									</a>
								<%} %>
								</td>
								</s:if>
